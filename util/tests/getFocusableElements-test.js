...
describe('getFocusableElements', () => {
  ...
  describe('getNextFocusable', () => {
    it('with no parameters, it gets the first focusable element in the document', () => {
      expect(getNextFocusable()).to.equal(focal.buttons(0).$root);
    });

    it('gets the next focusable element', () => {
      expect(getNextFocusable(focal.buttons(0).$root)).to.equal(focal.buttons(1).$root);
    });

    it('gets the next focusable within a container', () => {
      expect(getNextFocusable(focal.container.$root, true, true)).to.equal(focal.buttons(2).$root);
    });

    it('gets the next focusable excluding container contents', () => {
      expect(getNextFocusable(focal.container.$root, false)).to.equal(focal.buttons(4).$root);
    });

    it('loops to the beginning of the document', () => {
      expect(getNextFocusable(focal.buttons(4).$root)).to.equal(focal.buttons(0).$root);
    });
    ...
    it('returns same element if looping is false and last element provided', () => {
      expect(getNextFocusable(focal.buttons(4).$root, true, false, false)).to.equal(focal.buttons(4).$root);
    });
  });

  describe('getPreviousFocusable', () => {
    it('gets the previous focusable element', () => {
      expect(getPreviousFocusable(focal.buttons(1).$root)).to.equal(focal.buttons(0).$root);
    });

    it('gets the previous focusable within a container', () => {
      expect(getPreviousFocusable(focal.container.$root, true, true)).to.equal(focal.buttons(3).$root);
    });

    it('gets the previous focusable excluding container contents', () => {
      expect(getPreviousFocusable(focal.container.$root, false)).to.equal(focal.buttons(1).$root);
    });

    it('loops to the end of the document', () => {
      expect(getPreviousFocusable(focal.buttons(0).$root)).to.equal(focal.buttons(4).$root);
    });
    ...
    it('returns same element if looping is false and first element provided', () => {
      expect(getPreviousFocusable(focal.buttons(0).$root, true, false, false)).to.equal(focal.buttons(0).$root);
    });
  });
});
