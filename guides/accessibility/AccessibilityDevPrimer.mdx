...
Color contrast is an important aspect to ensure that the webpage is legible, even for users with color-blindness or other sight disabilities. WCAG 2.0 AA standard requires that elements on a page have a sufficient color contrast of 4.5:1 for normal text and 3:1 for large or bold text.([1.4.3](https://www.w3.org/TR/WCAG21/#contrast-minimum)) Check out the links below for a color contrast checking tool. See our in-repo [Color Contrast Tool](?path=/docs/guides-accessibility-color-contrast-tool--docs) for checking palette colors directly against stripes-components variables.
