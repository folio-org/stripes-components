...
Being overlapped. This image shows an example of this problem - controls in the list are cut off by the list's wrapping element.

<img style={{ border: '1px solid #AAA' }} src={usePortalFalse} alt="Problematic overlay control, cut off by the border of its container."/>

Each of these overlay components implements a `usePortal` prop that will render its menu to the overlay elements. We also export a `<StripesOverlayWrapper>` that will automatically apply
