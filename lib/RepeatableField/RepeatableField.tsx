...
type RepeatableFieldProps = {
  ...
  getFieldUniqueKey?: (...args: any[]) => any;
  hasMargin?: boolean;
...
const RepeatableField = ({
      ...
      mutations.forEach((mutation) => {
        ...
        if (hasBeenFocused) {
          if (
            ...
            // only apply to repeatable field item addition removal
  ...
  return (
    ...
    >
      {legend && (
        <Headline data-test-repeatable-field-legend tag="legend">
          {legend}
        </Headline>
      )}
      ...
      {fieldsLength === 0 && emptyMessage && (
        <EmptyMessage
          ...
          data-test-repeatable-field-empty-message
        >
          {emptyMessage}
        </EmptyMessage>
      )}
      ...
      {fieldsLength > 0 && (
        <ul className={css.repeatableFieldList} data-test-repeatable-field-list>
          ...
          {fields.map((field, index) => (
            ...
            >
              <RepeatableFieldContent rootRef={rootRef}>
                {renderField(field, index, fields)}
              </RepeatableFieldContent>
              {showDeleteBtn && (
                <div
                  className={`${css.repeatableFieldRemoveItem} ${headLabels ? css.repeatableFieldRemoveUnlabeledItem : ""}`}
                >
