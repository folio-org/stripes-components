...
type PaneHeaderProps = {
  ...
  paneSub?: string | React.ReactElement | React.ReactNode;
  paneTitle?: string | React.ReactElement | React.ReactNode;
...
const PaneHeader = ({
  ...
  const renderActionMenuContent = ({ onToggle, open, keyHandler }) => (
    <DropdownMenu>{actionMenu({ onToggle, open, keyHandler })}</DropdownMenu>
  );
  ...
  const getCenteredContentArea = () => (
    <div className={css.paneHeaderCenter}>
      <div className={css.paneHeaderCenterInner} id={`${_id}-pane-title`}>
        {paneTitle && (
  ...
  const getContentArea = (placement, menuElement, hasDismissibleIcon) => {
    ...
    if (hasDismissibleIcon) {
      content = React.cloneElement(
        content,
        {},
        [renderDismissibleButton()].concat(
  ...
  if (header) {
    return (
      <div data-test-pane-header-custom className={css.paneHeader}>
        {header}
      </div>
    );
  }
  ...
  return (
    ...
    >
      {getContentArea(
        "first",
        firstMenu,
        dismissible === true || dismissible === "first",
      )}
