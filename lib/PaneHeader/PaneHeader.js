...
class PaneHeader extends Component {
   ...
   * Action Menu
   */
  renderActionMenuToggle = ({ onToggle, triggerRef, keyHandler, open, ariaProps, getTriggerProps }) => (
    <Button
  ...
  renderActionMenuContent = ({ onToggle, open, keyHandler }) => {
  ...
  renderActionMenu = () => {
    ...
    return (
      <Dropdown
        hasPadding
        renderTrigger={this.renderActionMenuToggle}
        renderMenu={this.renderActionMenuContent}
      />
    );
  };
  ...
  getContentArea(placement, menuElement, hasDismissibleIcon) {
    ...
    if (hasDismissibleIcon) {
      content = React.cloneElement(
        content,
        {},
        [this.getDismissibleButton()].concat(React.Children.toArray(content.props.children))
      );
    }
    ...
    if (actionMenu && placement === 'last') {
      content = React.cloneElement(
        content,
        {},
        [this.renderActionMenu()].concat(React.Children.toArray(content.props.children))
      );
    }
