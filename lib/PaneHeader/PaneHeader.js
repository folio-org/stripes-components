  ...
  getDismissibleButton() {
    ...
    return (
      <IconButton
        ...
        icon="closeX"
        onClick={onClose}
        title={this.context.intl.formatMessage({ id: 'stripes-components.closePane' })}
        className={css.paneHeaderCloseIcon}
        ariaLabel={this.context.intl.formatMessage({ id: 'stripes-components.closeItem' }, { item: description })}
      />
    );
  }
        ...
        <NavListSection activeLink="">
          {
            actionMenuItems.map((item, index) => {
              ...
              return (
                <NavListItem {...rest} key={index}>{label}</NavListItem>
              );
            })
          }
        </NavListSection>
      ...
      <div>
        { paneTitle && (
          <h2 className={css.paneTitle} aria-describedby={paneSub && `${this._id}-subtitle`}>
            { appIcon && <span aria-hidden="true"><AppIcon size="small" app={appIcon.app} appIconKey={appIcon.key} /></span> }
            <span tabIndex="-1" ref={this.props.paneTitleRef} autoFocus={this.props.paneTitleAutoFocus} className={css.paneTitleLabel}>{paneTitle}</span>
            { paneActionMenu && <Icon icon={actionMenuOpen ? 'up-caret' : 'down-caret'} size="small" iconRootClass={css.paneActionMenuIcon} />}
          </h2>
          )
        }
        { paneSub && (<p title={paneSub} id={`${this._id}-subtitle`} className={css.paneSub}><span>{paneSub}</span></p>) }
      </div>
    );
    /**
     * Action Menu
