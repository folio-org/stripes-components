...
type ProductionErrorProps = {
  error?: string;
  mapStackTrace?: (...args: any[]) => any;
  onCopyError?: (...args: any[]) => any;
...
const ProductionError = ({
  ...
  const handleCopyStack = () => {
    if (typeof onCopyError === "function") {
      ...
      navigator.clipboard.writeText(mappedStack).then(() => {
        setErrorCopied(true);
  ...
  return (
    <>
      <div className={css.header} data-test-error-boundary-production-error>
        ...
        >
          {title || (
            <FormattedMessage id="stripes-components.ErrorBoundary.defaultTitle" />
          )}
        </Headline>
        <Headline
          tag="h2"
        ...
        >
          {subTitle || (
            <FormattedMessage id="stripes-components.ErrorBoundary.defaultSubTitle" />
          )}{" "}
          <TextLink
            element="button"
            type="button"
      ...
      >
        {resetButtonLabel || (
          <FormattedMessage id="stripes-components.ErrorBoundary.defaultButtonLabel" />
        )}
      </Button>
      <Modal
        label={intl.formatMessage({
          id: "stripes-components.ErrorBoundary.errorDetails",
        })}
