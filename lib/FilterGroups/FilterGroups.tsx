...
import { AccordionSet, Accordion, FilterAccordionHeader } from "../Accordion";
...
type FilterGroupsProps = {
  config: {
    ...
    values: (
      | string
      | { cql: string; displayName?: React.ReactNode; name: string }
    )[];
  }[];
  disableNames?: {};
  filters: Record<string, any>;
...
export function filters2cql(config, filters = "") {
  ...
  for (const groupName of Object.keys(groups)) {
    ...
    if (group && group.cql) {
      ...
      if (
        !(
          values.length === group.values.length &&
          !group.restrictWhenAllSelected
        )
      ) {
        const mappedValues = values.map((v) => {
          // If the field is a {name,cql} object, use the CQL.
          const obj = group.values.filter(
            (f) => typeof f === "object" && f.name === v,
          );
      ...
      console.warn(
        `Expected to find match for ${groupName} in filter-config`,
        config,
      );
    }
  }
...
const FilterGroups = (props: FilterGroupsProps) => {
      ...
      <AccordionSet>
        {config.map((group, index) => (
          <Accordion
            ...
            displayClearButton={Object.keys(filters).some((filter) =>
              filter.startsWith(group.name),
            )}
