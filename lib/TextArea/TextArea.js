...
class TextArea extends Component {
  ...
  endControlRef = React.createRef();

  moveEndContent = (dimensions) => {
    if (!this.endControlRef.current) {
      return;
    }
    ...
    this.endControlRef.current.style.right = `${-(resizeDiff - RESIZE_HANDLE_WIDTH)}px`;
  };
  ...
  render() {
    ...
    if ((!readOnly && clearField) || endControl) {
      endControlElement = (
        <div className={css.endControls} ref={this.endControlRef}>
