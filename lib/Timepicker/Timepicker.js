...
export const defaultOutputFormatter = ({ value, formats, timezone = 'utc', intl }) => {
  ...
  const timeOfDay = parsedTime.isValid()
    ? parsedTime
    : dayjs(value, formatList.find(candidate => dayjs(value, candidate, true).isValid()) || formatList[0]);

  // an incomplete/unparseable value (e.g. mid-typing) leaves `timeOfDay`
  // invalid; bail out here rather than handing dayjs.tz() a garbage string
  // below, which throws instead of returning an invalid value.
  if (!timeOfDay.isValid()) return '';

  // Anchor to today's UTC calendar date so the result only depends on the
  // value's hour/minute and `timezone`, not on the browser's local date --
  // otherwise the local and UTC calendar dates can differ (e.g. near
  // midnight UTC), and if that difference spans a DST boundary in the
  // target zone, the resulting offset -- and the emitted time -- is off by
  // an hour.
  const normalizedValue = `${dayjs.utc().format('YYYY-MM-DD')} ${timeOfDay.format('HH:mm')}`;
  const isoTime = timezone === 'utc'
    ? dayjs.utc(normalizedValue, 'YYYY-MM-DD HH:mm')
    : dayjs.tz(normalizedValue, 'YYYY-MM-DD HH:mm', timezone);

  return isoTime.isValid() ? isoTime.toISOString().split('T')[1] : '';
};
...
const Timepicker = ({
  ...
  // previously, this set the global dayjs locale (`dayjs.locale(timeLocale)`)
  // so that dayjs's own locale data (day-period labels, etc.) would follow
  // this Timepicker. That's no longer necessary now that parsing/formatting
  // (parseLocalizedTime, formatLocalizedTime, getHourForLocalizedPeriod) is
  // driven directly by `Intl`, with `timeLocale` passed through explicitly.
