...
class Timepicker extends React.Component {
  constructor(props, context) {
    ...
    if (typeof this.props.input === 'undefined') {
      if (this.props.value === 'today' && typeof this.props.value === 'undefined') {
      ...
      inputValue = moment(this.props.input.value).format(this._timeFormat);
    }
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    ...
    if (nextProps.input) {
      if (nextProps.input.value !== '' && nextProps.input.value !== this.props.input.value) {
        initDate = moment(nextProps.input.value, this._timeFormat);
        inputValue = moment(nextProps.input.value).format(this._timeFormat);
        this.setState({
          timeString: inputValue,
        });
      } else if (nextProps.input.value === '' && nextProps.input.value !== this.props.input.value) {
        initDate = moment().format(this._timeFormat);
        inputValue = '';
        this.setState({
          timeString: inputValue,
        });
      ...
      if (nextProps.value === 'today' && nextProps.value === 'undefined') {
        initDate = moment();
        inputValue = initDate.format(this._timeFormat);
      } else {
        initDate = moment(nextProps.value, this._timeFormat);
        inputValue = initDate.format(this._timeFormat);
      }
      this.setState({
        timeString: inputValue,
  ...
  handleSetTime(e, hours, minutes, ampm) {
    ...
    if (e === null || e.type === 'click' || e.type === 'keydown') {
      ...
      if (moment(tempString, this._timeFormat, true).isValid()) {
        this.setState({
  ...
  handleFieldChange(e) {
    const hoursMins = e.target.value.split(/[:\s]/g);
    ...
    if (hoursMins.length < 3) {
      this.handleSetTime(e, hours, mins, 'PM');
      return;
    }
    const ampm = hoursMins[2];
  ...
  standardizeTime(time) { // eslint-disable-line class-methods-use-this
    const isoTime = moment(time, 'HH:mm A').toISOString();
      ...
      <TetherComponent {...mergedTetherProps}>
        ...
        <RootCloseWrapper onRootClose={this.handleRootClose} >
          <TimeDropdown
            ...
            selectedDate={this.state.timeString}
