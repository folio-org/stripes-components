...
const defaultParser = (value, timezone, timeFormat, passThroughValue, intl) => {
  ...
  if (containsUTCOffset(value)) {
    ...
    time = dayjs.utc(value, timeFormat).local();
  } else {
    ...
    */
    if (containsUTCOffset(value)) {
      ...
      valueTimeObject = dayjs(
        value,
        [timeFormat, timeFormat.includes('A') ? twelveHourFormats : twentyFourHourFormats],
        true
      );
    }
    ...
    if (valueTimeObject.isValid()) {
      ...
      // if the date's not valid, we just update the datestring to reflect user input...
