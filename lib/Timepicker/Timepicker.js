...
class Timepicker extends React.Component {
  constructor(props) {
    ...
    this.hourField = React.createRef(null);
    this.picker = null;
    this.srSpace = null;

    this.focusHourField = this.focusHourField.bind(this);
    this.handleSetTime = this.handleSetTime.bind(this);
    this.handleKeyDown = this.handleKeyDown.bind(this);
    this.clearTime = this.clearTime.bind(this);
    this.handleFieldClick = this.handleFieldClick.bind(this);
    this.toggleTimepicker = this.toggleTimepicker.bind(this);
    this.debouncedToggleTimepicker = debounce(this.toggleTimepicker, 100);
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (props.input) { // if we're using redux-form....
      ...
      if (props.input.value !== '' && props.input.value !== state.prevInputValue) {
        if (props.input.value === props.passThroughValue) {
          newState.timeString = moment.tz(state.timeZone).format(state.timeFormat);
          newState.presentedValue = props.passThroughValue;
        }
        // if value is blank....
      } else if (props.input.value === '' && props.input.value !== state.prevInputValue) {
      ...
      if (props.value === props.passThroughValue) {
        newState.timeString = moment.tz(state.timeZone).format(state.timeFormat);
        newState.presentedValue = props.passThroughValue;
      } else {
        newState.timeString = moment.tz(state.timeZone).format(state.timeFormat);
  ...
  componentDidUpdate(prevProps, prevState) {
    if (!prevState.showTimepicker && this.state.showTimepicker) {
      document.addEventListener('click', this.handleRootClose, true);
      setTimeout(() => this.focusHourField());
    } else if (prevState.showTimepicker && !this.state.showTimePicker) {
      document.removeEventListener('click', this.handleRootClose, true);
    }
  }

  focusHourField = () => {
    this.hourField.current?.focus(); // eslint-disable-line
  }
  ...
  handleFieldClick(e) {
    this.debouncedToggleTimepicker(e);
  }

  handleRootClose(e) {
    if (!this.dropdownRef.current?.contains(document.activeElement) &&
    !this.dropdownRef.current?.contains(e.target)) {
      this.debouncedToggleTimepicker();
    }
  }
  ...
  render() {
          ...
          <FormattedMessage id="stripes-components.showOrHideTimepicker">
            {([showOrHideTimepickerAriaLabel]) => (
              <IconButton
                ...
                onClick={this.debouncedToggleTimepicker}
    ...
    const renderDropdown = (
      <TimeDropdown
        ...
        firstFieldRef={this.hourField}
        onHide={this.debouncedToggleTimepicker}
        ...
        onKeyDown={this.handleKeyDown}
        onBlur={this.debouncedToggleTimepicker}
        locale={this.locale}
    ...
    if (tether) {
      ...
      return (
        <TetherComponent {...tetherProps}>
          {renderTextField}
          {this.state.showTimepicker && renderDropdown}
        </TetherComponent>
      );
    }
