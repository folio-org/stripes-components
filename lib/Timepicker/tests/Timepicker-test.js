...
import Timepicker, { convertTo24hr, defaultOutputFormatter, defaultParser } from '../Timepicker';
...
describe('Timepicker', () => {
  ...
  describe('reopening a zh-TW time with a 24-hour numeric value', () => {
    const localizedTimepicker = TimepickerInteractor({ id: 'zh-tw-timepicker-test' });

    beforeEach(async () => {
      await mountWithContext(
        <Timepicker
          id="zh-tw-timepicker-test"
          locale="zh-tw"
          timeFormat="A HH:mm"
          value="下午20:28"
          parser={value => value}
    ...
    it('retains the selected time when the dropdown is reopened', async () => {
      await localizedTimepicker.clickDropdownToggle();
      await localizedTimepicker.clickDropdownToggle();
      await localizedTimepicker.clickDropdownToggle();

      await timeDropdown.has({ hour: 8, minute: 28 });
      await localizedTimepicker.clickDropdownToggle();
    });

    it('selects the localized afternoon period', async () => {
      await localizedTimepicker.clickDropdownToggle();

      return timeDropdown.has({ hour: 8, minute: 28, period: '下午' });
    });
  });
...
describe('localized meridiem parsing', () => {
  it('parses and formats zh-TW morning and afternoon values', () => {
    expect(defaultParser('上午4:20', 'UTC', 'Ah:mm', { locale: 'zh-TW' })).to.equal('上午4:20');
    expect(defaultParser('下午4:20', 'UTC', 'Ah:mm', { locale: 'zh-TW' })).to.equal('下午4:20');
    expect(defaultParser('下午22:40', 'UTC', 'Ah:mm', { locale: 'zh-TW' })).to.equal('下午10:40');
  });

  it('formats localized zh-TW input for the backend', () => {
    expect(defaultOutputFormatter({
      value: '下午4:20',
      formats: ['Ah:mm'],
      timezone: 'UTC',
      intl: { locale: 'zh-TW' },
    })).to.match(/^16:20:00\.000Z$/);
