...
type TimepickerProps = {
  ...
  disabled?: boolean;
  id?: string;
  inputRef?: { current?: React.ReactElement } | ((...args: any[]) => any);
...
const containsUTCOffset = (value) => {
  ...
  const offsetZRE = /Z$/i;
  return (
    offsetRegex.test(value) || offsetRE2.test(value) || offsetZRE.test(value)
  );
};
...
const defaultParser = (value, timezone, timeFormat, intl) => {
...
// overrides to the props using this as default.
const defaultOutputFormatter = ({ value, formats, timezone = "utc", intl }) => {
  // eslint-disable-line no-unused-vars
...
const Timepicker = ({
  ...
  const [timePair, updateTimePair] = useState({
    ...
    formatted: valueProp
      ? outputFormatter({
          value: valueProp,
          timezone,
          formats: timeFormat.includes("A")
            ? twelveHourFormats
            : twentyFourHourFormats,
        })
      : "",
  });
  ...
  useEffect(() => {
    if (
      ...
      candidate.current = Object.assign(
        candidate.current,
  ...
  maybeUpdateValue = (value) => {
     ...
     */
    const additionalTimeFormats = timeFormat.includes("A")
      ? twelveHourFormats
      : twentyFourHourFormats;
    if (containsUTCOffset(value)) {
      valueTimeObject = moment
        .utc(value, [timeFormat, ...additionalTimeFormats])
        .local();
    } else {
      // use strict mode to check validity  - incomplete dates, anything not conforming to the format will be invalid
      valueTimeObject = moment(value, [timeFormat], true);
    }
  ...
  const handleChange = (e) => {
    ...
    if ((!useInput || !outputBackendValue) && onChange) {
      ...
      typeof candidate.current.formatted === "string" &&
  ...
  const handlePickTime = ({ hour, minute, period, dayPeriods }) => {
    const adjustedHour = dayPeriods
      ? convertTo24hr(hour, period, dayPeriods)
      : hour;
    const displayTime = moment({ hour: adjustedHour, minute }).format(
      timeFormat,
    );
  ...
  if (readOnly || disabled) {
    ...
    ariaLabel = intl.formatMessage(
      { id: "stripes-components.Timepicker.ScreenReaderLabel" },
  ...
  const endElements = [
    <FormattedMessage
      key="pickButton"
      id="stripes-components.showOrHideTimepicker"
    >
    ...
    <>
      ...
      >
        ...
        <TextField
          ...
          endControl={
            inputProps.readOnly || inputProps.disabled ? null : (
              <>{endElements}</>
            )
          }
...
export default FormField(Timepicker, ({ input, meta }) => ({
  onBlur: input?.onBlur,
