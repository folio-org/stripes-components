...
class TimeDropdown extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!nextProps.visible && nextProps.visible !== this.props.visible) {
      this._focusTimeout = setTimeout(() => { this.props.mainControl.focus(); }, 20);
    }
  }
  ...
  buildState(props){
    ...
    if (props.selectedTime === '') {
      ...
      if (moment(props.selectedTime, props.timeFormat).isValid()) {
        initMoment = moment(props.selectedTime, props.timeFormat, true);
    ...
    return (
      <div role="form" onBlur={this.handleBlurDropdown} ref={(c) => { this._pickerContainer = c; }} className={this.getRootClass()} id={`timeDD-${this.props.id}`} aria-hidden={!this.props.visible}>
        <Row center={this.state.hoursFormat === '12' ? undefined : 'xs'}>
          <Col xs={4} className={css.center}><IconButton tabIndex="-1" icon="up-arrow" id={`clickable-timeDD-${this.props.id}-next-hour`} onClick={() => { this.incrementTime(1, 'hour', true); }} /></Col>
          <Col xs={1}>&nbsp;</Col>
          <Col xs={4} className={css.center}><IconButton tabIndex="-1" icon="up-arrow" id={`clickable-timeDD-${this.props.id}-next-minute`} onClick={() => { this.incrementTime(this.props.minuteIncrement, 'minute', true); }} /></Col>
        </Row>
        ...
        <Row center={this.state.hoursFormat === '12' ? undefined : 'xs'}>
          <Col xs={4} className={css.center}><IconButton tabIndex="-1" id={`clickable-timeDD-${this.props.id}-prev-hour`} icon="down-arrow" onClick={() => { this.incrementTime(1, 'hour', false); }} /></Col>
          <Col xs={1}>&nbsp;</Col>
          <Col xs={4} className={css.center}><IconButton tabIndex="-1" id={`clickable-timeDD-${this.props.id}-prev-minute`} icon="down-arrow" onClick={() => { this.incrementTime(this.props.minuteIncrement, 'minute', false); }} /></Col>
        </Row>
        <Row>
          <Col xs={12}>
            <Button
