...
import {
  dayjs,
  getLocalizedTimePeriod,
  getLocalizedTimePeriodInfo,
  parseLocalizedTime
...
* @returns {string}
*/
const deriveDefaultTimePeriod = (value, hoursFormat, timeFormat, locale, dayPeriods) => {
  if (hoursFormat === '24') return null;
  if (value) {
    const parsedTime = parseLocalizedTime(value, timeFormat, locale);
    const period = parsedTime.isValid() && getLocalizedTimePeriod(parsedTime.hour(), locale);
    if (period) return period;
  }
  return getLocalizedTimePeriod(dayjs().hour(), locale) || dayPeriods[0];
};
...
const TimeDropdown = ({
  ...
  const dayPeriods = hoursFormat === '12'
    ? getLocalizedTimePeriodInfo(locale).map(period => period.value)
    : null;
  const hoursFormatCharacter = useRef(`${hoursFormat === '24' ? 'HH' : 'h'}`).current;
  const selectedTimeObject = selectedTime ? parseLocalizedTime(selectedTime, timeFormat, locale) : null;
  const currentTimeObject = dayjs();
  const [hour, setHour] = useState(
    selectedTimeObject?.isValid() ?
      selectedTimeObject.format(hoursFormatCharacter) :
      currentTimeObject.format(hoursFormatCharacter)
  );
  const [minute, setMinute] = useState(
    selectedTimeObject?.isValid() ?
      selectedTimeObject.format('mm') :
      currentTimeObject.format('mm')
  );
  const [period, setPeriod] = useState(deriveDefaultTimePeriod(
    selectedTime,
    hoursFormat,
    timeFormat,
    locale,
