...
class Popover extends React.Component {
  ...
  handleToggle() {
    this.setState((curState) => {
      ...
      if (!open) {
        nextState.open = true;
        this.timer = setTimeout(() => this.setState({ visible: true }), 25);
      } else {
        nextState.visible = false;
        this.timer = setTimeout(() => this.setState({ open: false }), 100);
      }
      return nextState;
    });
  }
        ...
        switch (child.props['data-role']) {
          case 'target': {
            ...
            this.rendered_target = (
              <div className={classnames(css.popoverTarget)} ref={(t) => { this._target = t; }} key="target">
                {target}
              </div>
            );
            break;
          ...
          case 'popover': {
            ...
            this.rendered_popover = (
              <RootCloseWrapper onRootClose={this.handleToggle}>
                <div tabIndex="-1" role="dialog" className={classnames(css.popoverPop, { [css.noPadding]: this.props.noPadding })} ref={(t) => { this._popover = t; }}>
                  {popover}
                </div>
              </RootCloseWrapper>
            );
  ...
  render() {
    ...
    if (this.state.open) {
      popover = ReactDOM.createPortal(
        <Transition key="popover" timeout={70} in={this.state.visible}>
          { status => (
