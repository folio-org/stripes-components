...
type LegacyPopoverProps = {
  activeClass?: string;
  alignment?: "middle" | "center" | "start" | "end";
...
class LegacyPopover extends React.Component<LegacyPopoverProps> {
  constructor(props) {
    super(props);
  ...
  getAlignment(targetRect, popRect, position, alignment) {
    ...
    switch (alignment) {
      ...
      case "end": {
        a =
          targetRect[coordinate] - (popRect[dimension] - targetRect[dimension]);
        alignObject[coordinate] = a;
        break;
      }
      default:
  ...
  updatePosition() {
    this.popRAF = requestAnimationFrame(() => {
      ...
      const newAlign = this.getAlignment(
        targetRect,
        popRect,
        derivedPosition,
    ...
    if (!this._target) {
      React.Children.forEach(this.props.children, (child) => {
        switch (child.props["data-role"]) {
          ...
          case "popover": {
            const popover = React.cloneElement(child, child.props);

            this.rendered_popover = (
              <RootCloseWrapper
                onRootClose={this.handleToggle}
                ref={this.popoverRef}
              >
                <div
                  ...
                  tabIndex="-1"
                  role="dialog"
                  className={classnames(css.popoverPop, {
  ...
  render() {
    ...
    return (
      <span
        className={classnames({
          [this.props.activeClass]: this.props.activeClass && this.state.open,
        })}
      >
        {this.rendered_target}
