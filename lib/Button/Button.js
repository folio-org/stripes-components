...
function Button(props) {
  ...
  const { buttonStyle, bottomMargin0, marginBottom0, align, hollow, fullWidth, bsRole, bsClass, onClick, allowAnchorClick, ...buttonProps } = props;

  if (props.href) {
    return (
      // eslint-disable-next-line jsx-a11y/no-static-element-interactions
      <a className={getStyle()} onClick={handleAnchorClick} {...buttonProps}>
        {props.children}
      </a>
    );
  }
  ...
  return (
    <button className={getStyle()} onClick={onClick} {...buttonProps}>
      {props.children}
    </button>
  );
}
