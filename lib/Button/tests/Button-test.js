...
import { Button as Interactor, converge, runAxeTest } from '@folio/stripes-testing';
...
describe('Button', () => {
  ...
  describe('rendering a button', () => {
    ...
    it('buttonRef callback returns a HTML button element', () => {
      converge(() => expect(ref instanceof Element).to.be.true);
      converge(() => expect(ref.tagName).to.equal('BUTTON'));
    });
  });
