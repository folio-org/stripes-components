...
type PasswordStrengthProps = {
  input?: { value?: string };
  inputColProps?: Record<string, any>;
  meta?: { dirty?: boolean; valid?: boolean };
  passwordMeterColProps?: Record<string, any>;
  passwordStrengthHidden?: boolean;
};
...
class PasswordStrength extends Component<PasswordStrengthProps> {
  static defautlProps = {
    inputColProps: {},
  ...
  initPasswordStrength() {
    import("tai-password-strength").then((taiPasswordStrength) => {
      ...
      this.strengthTester.addCommonPasswords(
        taiPasswordStrength.commonPasswords,
      );
  ...
  render() {
    ...
    const { strengthCode } =
      (hasLoaded && this.strengthTester.check(value)) || {};
    const output =
      this.outputTypes[strengthCode] || defaultPasswordStrengthType;
    ...
    return (
      <Row>
        ...
        <Col xs={4} aria-live="polite" {...passwordMeterColProps}>
          {isVisible && (
            <div className="password-strength">
              <div className={css["password-strength__label"]}>
                <FormattedMessage id="stripes-components.passwordStrength.label" />
              </div>
              <div
                className={classNames(
                  css.indicator__container,
                  css[`indicator__container--${output}`],
                )}
              >
              ...
              <div className={css["password-strength-text__wrapper"]}>
                ...
                <InfoPopover
                  content={
                    <FormattedMessage id="stripes-components.passwordStrength.infoPopoverText" />
                  }
                />
              </div>
            </div>
