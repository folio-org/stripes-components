...
export default class Tooltip extends Component {
  ...
  componentDidMount() {
    ...
    if (this.triggerRef && this.triggerRef.current) {
      ...
      this.triggerEl.addEventListener('touchstart', this.handleTouchEvent, true);
  ...
  componentWillUnmount() {
    ...
    if (this.triggerEl) {
      ...
      this.triggerEl.removeEventListener('keydown', this.handleKeyDown, true);
      this.triggerEl.removeEventListener('touchstart', this.handleTouchEvent, true);
    }
  }

  /**
   * Check if the user is using a touch device
   * This information can be used in combination with the 'hideOnTouch'-prop
   * to prevent showing tooltips on touch devices
   */
  handleTouchEvent = () => {
    this.setState({
      isTouch: true,
  ...
  toggle = (bool) => {
    ...
    const { open, isTouch } = this.state;
    if (bool !== open && !(hideOnTouch && isTouch)) {
      this.setState({
        open: bool,
      }, () => {
        if (!this.state.open) {
          // Reset isTouch after closing a tooltip
          // because some users might be using a computer that supports both touch and mouse
          this.setState({
            isTouch: false
          });
        }
