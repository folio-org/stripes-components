...
export default class Tooltip extends Component<TooltipProps> {
  ...
  toggle = (bool) => {
    ...
    if (!bool) {
      if (
        !this.overlayRef?.current?.matches(":hover") &&
  ...
  renderProximityElement = () => {
    ...
    return (
      <span className="sr-only" data-test-tooltip-proximity-element>
        {text && (
          <span role="tooltip" id={ariaIds.text}>
            {text}
          </span>
        )}
        {sub && (
          <span role="tooltip" id={ariaIds.sub}>
            {sub}
          </span>
        )}
      </span>
  ...
  validateTriggerElement = () => {
    ...
    const ariaIds = Object.keys(this.getAriaIds()).map(
      (key) => this.getAriaIds()[key],
    );
    ...
    const hasCorrectIds =
      ariaIds.filter((id) => triggerAriaValues.includes(id)).length ===
      ariaIds.length;

    // // Make it clear for the developer that something is wrong
    if (!hasCorrectIds) {
      console.warn(
        "For accessibility reasons, your <Tooltip> trigger element:",
        this.triggerEl,
        "should include one or both of the following attributes: aria-labelledby",
        "and/or aria-describedby with the relevant ID(s):",
        ariaIds,
        ".",
