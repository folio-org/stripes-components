...
class AddressList extends React.Component {
  ...
  render() {
    ...
    if (this.props.canEdit) {
      addAddressButton = (<Button onClick={this.handleAddNew} marginBottom0><FormattedMessage id="stripes-components.addNew" /></Button>);
    }
    ...
    return (
      <div>
        ...
        {addAddressButton}
        <div aria-label={formatMsg({ id: 'stripes-components.addressSection' })} tabIndex="0" role="tabpanel" ref={(ref) => { this.container = ref; }}>
          ...
          { canToggleExpansion &&
            <div style={{ marginTop: '8px' }}>
              <Row>
                <Col xs={12}>
                  <Button fullWidth onClick={this.onToggleExpansion}>{(!this.state.expanded && <Layout className="flex centerContent"><Icon icon="down-arrow" /><FormattedMessage id="stripes-components.showMoreAddresses" /> ({addresses.length - 1})</Layout>) || <Layout className="flex centerContent"><Icon icon="up-arrow" /><FormattedMessage id="stripes-components.showLessAddresses" /> </Layout>}</Button>
                </Col>
              </Row>
            </div>
          }
...
AddressList.contextTypes = contextTypes;
