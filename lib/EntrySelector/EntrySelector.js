...
class EntrySelector extends React.Component {
  ...
  renderDetail(item) {
    ...
    const lastMenu = (
      <Button id="clickable-edit-item" onClick={() => this.props.onEdit(item)} title={this.props.stripes.intl.formatMessage({id: "stripes-core.button.edit"})} buttonStyle="primary">
        {this.props.stripes.intl.formatMessage({id: "stripes-core.button.edit"})}
      </Button>
    );
    ...
    const LastMenu = addMenu || (
      <PaneMenu>
        <Button title={addButtonTitle} onClick={this.props.onAdd} buttonStyle="primary paneHeaderNewButton">
          {this.props.stripes.intl.formatMessage({id: "stripes-components.new"})}
        </Button>
      </PaneMenu>
    );
