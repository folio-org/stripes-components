...
describe('MetaSection', () => {
  ...
  describe('When useAccordion false', () => {
    const metaSectionWOAccordion = new Interactor();

    describe('If no update details are provided', () => {
      beforeEach(async () => {
        await mountWithContext(
          <MetaSection 
            useAccordion={false}
      ...
      it('Should render "Unknown user" updated-by field',
        () => metaSectionWOAccordion.has({ updatedByText: including('Unknown user') }));
    });
      
    describe('when update details are provided', () => {
      beforeEach(async () => {
        await mountWithContext(
          <MetaSection
            lastUpdatedDate="2021-07-08T00:00:00Z"
      ...
      it('Should render updated-by field',
        () => metaSectionWOAccordion.has({ updatedByText: including('Opus') }));

      it('Should render not render created by details',
        () => metaSectionWOAccordion.has({ createdByText: undefined }));

      it('renders with default class', async () => {
        await metaSectionWOAccordion.perform((e) => expect(e.classList.contains(css.metaSectionInline)).to.be.true);
      });
    })
  })
});
