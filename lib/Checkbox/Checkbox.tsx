...
class Checkbox extends React.Component<CheckboxProps> {
  ...
  renderLabelInfo = () => {
    const { labelInfo, labelInfoClass } = this.props;
    ...
    return (
      <span
        ...
        className={classNames([`${css.checkBoxLabelInfo}`], labelInfoClass)}
      >
        {labelInfo}
      </span>
    );
  ...
  render() {
    ...
    const [, inputCustom] = separateComponentProps(
      this.props,
      Checkbox.propTypes,
    );

    return (
      <div data-test-checkbox className={this.getRootClasses()}>
        <Label
        ...
        <div role="alert">{this.renderFeedbackElement()}</div>
      </div>
    );
  }
}

export default formField(Checkbox, ({ meta }) => ({
