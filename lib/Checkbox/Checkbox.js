...
class Checkbox extends React.Component {
  ...
  getRootClasses = () => {
    const { className, fullWidth, vertical, label, disabled, inline } = this.props;
  ...
  render() {
    ...
    // Only render a <label> element if we have a label value to display
    const Element = label ? Label : 'span';

    // Only apply the for-attribute if we render a <label> element
    const htmlFor = label ? this.id : undefined;

    return (
      <div className={this.getRootClasses()}>
        <Element htmlFor={htmlFor} className={this.getLabelClasses()}>
