...
import AccessibleFocusCss from '../AccessibleFocus/AccessibleFocus.css';
...
const propTypes = {
  ...
  accessibleFocusProps: PropTypes.object,
...
class Checkbox extends React.Component {
  constructor(props) {
    ...
    // If no ID is provided we add a random ID
    // (id is required for Checkboxes to work properly)
    this.inputId = this.props.id || uniqueId('checkbox-');
  ...
  render() {
    ...
    const accessibleFocusOptions = {
      boxOffsetStart: label ? 'small' : undefined,
      boxOffsetEnd: label ? 'small' : undefined,
      ...this.props.accessibleFocusProps,
    };

    return (
      <div className={this.getRootStyle()}>
        <AccessibleFocus {...accessibleFocusOptions} className={this.getLabelStyle()} component="label" role="button">
          <input
            type="checkbox"
            {...inputAriaProps}
          ...
          <div className={css.focusEl}><AccessibleFocus {...accessibleFocusOptions} component="div" focused /></div>
