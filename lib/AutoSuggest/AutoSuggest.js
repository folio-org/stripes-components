...
const AutoSuggest = ({
  ...
  const testId = useRef(id || uniqueId('autoSuggest-')).current;
  ...
  const {
    ...
    onInputValueChange: ({ inputValue: newValue }) => onChange(newValue),
  ...
  return (
    <div className={css.downshift} data-test-autosuggest aria-expanded={isOpen}>
      {control}
      <Popper {...popper} hideIfClosed isOpen={isOpen} anchorRef={container}>
        {list}
      </Popper>
    </div>
  )
