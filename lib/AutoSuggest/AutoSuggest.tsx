...
const getClass = ({ item, index, selectedItem, highlightedIndex }) =>
  classNames([
    `${css.autoSuggestItem}`,
...
const defaultIncludeItem = (item, searchString) =>
  item.value.includes(searchString);
...
const AutoSuggest = ({
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) => !filterValue || includeItem(item, filterValue)),
    [filterValue, items],
  );
...
export default formField(AutoSuggest, ({ meta }) => ({
  dirty: meta.dirty,
