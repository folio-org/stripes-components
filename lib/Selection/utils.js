...
export const getSelectedObject = (value, dataOptions = []) => {
  if (dataOptions.length > 0) {
    if (typeof value !== "undefined") {
      ...
      const valueIndex = dataOptions.findIndex((o) => o.value === value);
      return dataOptions[valueIndex];
    }
  }
};
...
export const filterOptionList = (value, dataOptions) => {
  ...
  // results inserted.
  const grouped = dataOptions.some((o) => o.hasOwnProperty("options"));
  if (grouped) {
    return dataOptions.reduce((options, opt) => {
      // global, ungrouped options
      if (opt.hasOwnProperty("value")) {
        if (valueRE.test(o.label)) {
          return [...options, opt];
        }
...
export const ensureValuedOption = (index, dataOptions) => {
  if (dataOptions[index]) {
    if (dataOptions[index].hasOwnProperty("value")) {
...
export const reduceOptions = (dataOptions) =>
  dataOptions?.reduce((options, op) => {
    if (op.value) options.push(op);
    if (op.options) options = [...options, ...op.options];
    return options;
  }, []);
