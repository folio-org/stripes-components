...
class SingleSelect extends React.Component { // eslint-disable-line react/no-deprecated
  ...
  filterList(e) {
    ...
    if (e.target.value === '') {
      this.setState({
        cursoredIndex: this.state.selectedIndex || 0,
        cursoredLabel: this.state.selectedLabel || dataOptions[0].label,
        cursoredValue: this.state.selectedValue || dataOptions[0].value,
        searchTerm: '',
        renderedList: this.props.dataOptions,
        filtering: false,
      });
    ...
    if (presented.length === 0) {
      ...
      ariaMessage = this.props.intl.formatMessage(
        { id: 'stripes-components.selection.filterResultFeedback' },
        { length: presented.length, label: presented[0].label }
      );
    }
  ...
  render() {
    ...
    const renderedSelectionId = `selected-${this.testId}-item`;
    let requiredElement = null;
    let requiredId = '';
    if (this.props.required) {
      requiredId = `sl-${this.testId}-required`;
      requiredElement = (<span className="sr-only" id={requiredId}>required</span>);
    }

    let labelledBy;
    if (this.props.label) {
      labelledBy = `sl-label-${this.testId} ${renderedSelectionId} ${requiredId}`;
    } else {
    ...
    const selectList = (
      <SelectList
        ...
        emptyMessage={this.props.emptyMessage ||
          this.props.intl.formatMessage({ id: 'stripes-components.selection.noMatches' })
        }
