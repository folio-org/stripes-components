...
class SingleSelect extends React.Component { // eslint-disable-line react/no-deprecated
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.showList !== prevState.showList && !this.state.showList) {
      this.control.focus();
    }
  }
  ...
  handleFieldClick() {
    if (this.state.showList) {
      ...
      this.showList();
      // this._filterFocusTimeout = setTimeout(() => { this.filterTextInput.focus(); }, 15);
    }
  }
  ...
  handleRootClose() {
    if (this.state.showList) {
      ...
      if (!contains(this.container, document.activeElement) && 
          !contains(this.selectList, document.activeElement)) {
  ...
  render() {
    ...
    let renderedSelectionId = `selected-${this.testId}-item`;
    let requiredElement = null;
    if (this.props.required) {
      const requiredId = `sl-${this.testId}-required`;
      requiredElement = (<span className="sr-only" id={requiredId}>required</span>);
      renderedSelectionId =`${requiredId} ${renderedSelectionId}`;
    }

    let labelledBy;
    if (this.props.label) {
      labelledBy = `sl-label-${this.testId} ${renderedSelectionId}`;
    } else {
      labelledBy = renderedSelectionId;
    }
      ...
      >
        ...
        <div className={css.SelectionControlContainer}>
          <button
            ...
            id={this.testId}
            aria-invalid={(this.props.errorText || this.props.warningText)}
...
export default reduxFormField(SingleSelect,
  ({ input, meta }) => ({
    value: input.value,
    name: input.name,
    ...
    warningText: (meta.dirty ? meta.warning : undefined),
    errorText: (meta.dirty ? meta.error : undefined),
    isValid: (meta.dirty && meta.valid),
