...
const getItemClass = (item, i, props) => {
  ...
  const grouped = optionIsGrouped(item, dataOptions)
    ? " " + css.groupedOption
    : "";
...
const getClass = ({
  ...
  validationClasses = classNames(
    ...
    {
      [`${formStyles.isValid}`]: useValidStyle && valid && !error && !warning,
    },
  );
...
const Selection = ({
  ...
  useEffect(() => {
    ...
    if (
      dataOptions?.length !== dataLength.current &&
      value &&
      selectedItem === null
    ) {
  ...
  const renderedOptionsFn = useCallback(
    function optionRenderer(data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (item.value !== undefined) {
          ...
          rendered.push(
            <li key={`${item.label}-heading-${i}`} className={css.groupLabel}>
              {formatter({ option: item, searchTerm: filterValue })}
            </li>,
          );
        }
  ...
  const renderedOptions = useMemo(
    ...
    [flattenedOptions, renderedOptionsFn],
  );
  ...
  const renderOptions = useCallback(() => {
    ...
    if (loading || filterValue !== debouncedFilterValue) {
      return (
        <li role="option" className={css.option} aria-selected="false">
    ...
    if (options.length === 0) {
      return (
        <li role="option" className={css.option} aria-selected="false">
          <span>
            -{formatMessage({ id: "stripes-components.selection.emptyList" })}-
          </span>
        </li>
      );
    }
      ...
      <div className={css.selectionControlContainer}>
        <div className={formStyles.inputGroup}>
          ...
          >
            ...
            <div className={css.singleValue} id={valueId}>
              {valueLabel}
            </div>
          </button>
...
export default formField(Selection, ({ meta }) => ({
  dirty: meta.dirty,
