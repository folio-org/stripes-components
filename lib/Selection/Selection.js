...
const Selection = ({
  ...
  const dbUpdateFilter = useRef(debounce((filter) => {
    // console.log(`actually filtering data: ${filter}`)
    updateDebouncedFilter(filter)
  }, 300)).current;
  const filterUpdateFn = useRef(function filterUpdater(filter) {
    // console.log(`just updating filter value: ${filter}`)
  ...
  const filterFn = useCallback(function filter(data) {
    // console.log(`filtering... ${debouncedFilterValue}`);
    return onFilter(debouncedFilterValue, data);
  }, [debouncedFilterValue, onFilter]);

  const flatten = useRef(function flattener(data) {
  ...
  const { loading: filtering, result: options } = useProcessedData(
    dataOptions,
    filterFn,
    noop,
    () => {
      if (dataOptions.length === 0) return false;
      if (debouncedFilterValue !== filterValue) return false;
      return (!asyncFilter || !debouncedFilterValue)
    },
  );
  ...
  const { loading: reducing, result: reducedListItems } = useProcessedData(
    options,
  ...
  const { loading: flattening, result: flattenedOptions } = useProcessedData(
    options,
    flatten.current,
    undefined,
    () => !filtering && options.length > 0,
  ...
  const renderedOptionsFn = useCallback(function optionRenderer(data) {
    // if (data.length > 0) { console.log(`rendering options for filter: ${filterValue}`) };
  ...
  const { loading: optionsProcessing, result: renderedOptions } = useProcessedData(
    flattenedOptions,
    renderedOptionsFn,
    () => {
      setUpdate(cur => {
    ...
    () => (!reducing || !flattening) && flattenedOptions.length > 0
  );
  ...
  const renderOptions = useCallback(() => {
    ...
    /* loading message */
    if (loading || optionsProcessing || flattening || filtering) {
      return (
        <li
          role="option"
