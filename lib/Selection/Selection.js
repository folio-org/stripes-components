...
const Selection = ({
  ...
  const asyncFiltering = useRef(typeof asyncFilter === 'function' ? asyncFilter() : asyncFilter).current;
  ...
  const options = useMemo(() => {
    return (asyncFiltering || !debouncedFilterValue) ? dataOptions : filterFn(dataOptions)
  },
  ...
  useEffect(() => {
    if (asyncFiltering && debouncedFilterValue) {
      filterFn(dataOptions);
    }
  }, [debouncedFilterValue, asyncFiltering]);
...
Selection.propTypes = {
  asyncFilter: PropTypes.oneOfType([PropTypes.func, PropTypes.bool]),
