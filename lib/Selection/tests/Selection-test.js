...
describe('Selection, Single select', () => {
  ...
  describe('using with redux-form', () => {
    describe('with initial value', () => {
      beforeEach(async () => {
        await mountWithContext(
          <TestForm initialValues={{testSelection: listOptions[3].value}}>
            <Field
              name="testSelection"
              component={Selection}
              dataOptions={listOptions}
              validate={(value) => (value === listOptions[4].value ? 'testSelection is invalid' : undefined)}
            />
          </TestForm>
        );
      });
      ...
      it('sets initial value', () => {
        expect(selection.valLabel).to.equal(listOptions[3].label);
      });

      describe('changing the value', () => {
        beforeEach(async() => {
          await selection.expandAndClick(3);
        });

        it('applies the \'hasChanged\' styling', () => {
          expect(selection.hasChangedStyle).to.be.true;
        });

        it('does not apply the \'validation success\' styling', () => {
          expect(selection.hasValidStyle).to.be.false;
        });
      ...
      describe('changing to an invalid value', () => {
        beforeEach(async() => {
        ...
        it('applies the \'error\' styling', () => {
          expect(selection.hasErrorStyle).to.be.true;
        });
      });
    });
    describe('setting the "valid" style indicator', () => {
      beforeEach(async () => {
        await mountWithContext(
          <TestForm>
            <Field
      ...
      describe('changing to a valid value', () => {
        beforeEach(async () => {
          await selection.expandAndClick(4);
        });

        it('renders the "isValid" style', () => {
          expect(selection.hasValidStyle).to.be.true;
        });
      });
    });
  });
