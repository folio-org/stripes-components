...
const altOuterHandler = (e) => { e?.preventDefault(); action('altOuter')(); };
const altInnerHandler = (e) => { e?.preventDefault(); action('altInner')(); };
...
const BasicUsage = () => {
  ...
  const outerAttachRef = useRef(null);
  const innerAttachRef = useRef(null);
  ...
  return (
    <div>
      ...
      <HotKeys keyMap={outerK} handlers={outerH} id="outer" attach={outerAttachRef}>
        <div style={{ border: '1px dashed blue', padding: '10px' }} ref={outerAttachRef}>
          <TextField label="Outer field" value={value} onChange={(e) => { setValue(e.target.value); }} />
          <HotKeys keyMap={innerKeyMap} handlers={innerHandlers} id="inner" attach={innerAttachRef}>
            <div style={{ border: '1px dashed orange', padding: '10px' }} ref={innerAttachRef}>
