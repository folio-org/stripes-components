...
describe('Focused Prop: function', () => {
  ...
  describe('when a keyMap and a handler are provided to the same component', () => {
    ...
    it('then renders children without any wrapping divs', () => {
  ...
  describe('when a keyMap is provided to a parent component and a handler to a child component', () => {
    ...
    describe('and a child element of the component defining the handlers is focused', () => {
      ...
      it('renders the handlerChild input without a wrapper', () => {
        const html = wrapperRef.current.outerHTML;
        expect(html).to.equal('<div><div tabindex="-1" id="outer"><div><input aria-label="handlerChildElement" tabindex="-1"></div><input aria-label="keyMapChildElement"></div><input aria-label="siblingElement"></div>');
      });
  ...
  describe('when HotKey components defining handlers are nested inside each other', () => {
    ...
    describe('and a child element of the inner component is in focus', () => {
      ...
      it('renders the innerElement and outerElement without a wrapper', () => {
        const html = wrapperRef.current.outerHTML;
        expect(html).to.equal('<div><div tabindex="-1"><input aria-label="outerElement"><input aria-label="innerElement" tabindex="-1"></div></div>');
      });
