...
class HotKeys extends React.Component {
  ...
  componentDidMount() {
    ...
    // when we remove findDOMNode(), we should restore this warning, or
    // go a step further and throw an error when this.props.attach is missing
    // if (!this.attachment) {
    ...
    // if no attachment, re-render to pick up the attachment node in FocusTrap.
    if (attach === undefined) {
      this.forceUpdate();
    }
  }
  ...
  handlerWrapper = (e, handler, sequence) => {
    // AltGr is reported by the browser as the "AltGraph" modifier (often alongside
    // ctrlKey+altKey on Windows). Many international keyboard layouts use AltGr to
    // type diacritics/special characters (e.g. AltGr+E -> "ę"), which can otherwise
    // be misread as a ctrl/alt hotkey chord. Treat it as text input, not a shortcut.
  ...
  render() {
    ...
    // Prefer the "attach" prop directly - it's a stable ref (or DOM node) that
    // FocusTrap can safely resolve as soon as it mounts. Only fall back to
    // this.attachment (populated via findDOMNode() in componentDidMount, and
    // delivered by the forceUpdate() there) when no "attach" prop was given.
    const element = attach !== undefined ? attach : this.attachment;

    return (
      <HotKeysProvider value={this.state.HKcontext}>
        <FocusTrap {...focusTrapProps} onFocus={this.onFocus} onBlur={this.onBlur} element={element}>
          {children}
        </FocusTrap>
      </HotKeysProvider>
    );
