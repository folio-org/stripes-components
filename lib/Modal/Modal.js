...
import { MAIN_NAVIGATION_SELECTOR, OVERLAY_CONTAINER_SELECTOR } from '../../util/consts';
...
function getA11yLabel(props, labelId) {
  ...
  return hasAriaLabel ? { 'aria-label': props.ariaLabel || props['aria-label'] } :
...
// of the document.
function useAssistiveTechTrap() {
  const elementList = useRef([]);

  // collect elements to set/remove inert attribute from.
  ...
  elementList.current = [mainNav, ...overlaySiblings];

  const apply = useCallback(() => {
    elementList.current?.forEach(el => el?.setAttribute?.('inert', ''));
  }, [elementList])

  const release = useCallback(() => {
    elementList.current?.forEach(el => el?.removeAttribute?.('inert'));
  }, [elementList])
