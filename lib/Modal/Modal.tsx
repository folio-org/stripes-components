...
const propTypes = {
  ...
  scope: PropTypes.oneOf(["root", "module"]),
...
const Modal = forwardRef((props, ref) => {
  ...
  const getModalClass = () => classNames(css.modal, css[size]);

  const getContentClass = () => classNames(contentClass, css.modalContent);

  const focusEntered = useCallback(
    (e) => {
      if (!contentFocused) {
  ...
  const modalElement = open
    ? (status) =>
        ...
        */}
            {/* eslint-disable */}
            <div className={css.backdrop} onClick={handleBackgroundClick} />
            <div
              role="dialog"
