...
export function useAssistiveTechTrap() {
  const apply = () => {
    ...
    const overlaySiblings = Array.from(
      document.querySelectorAll(
        // siblings of the overlay container
...
const WrappingElement = forwardRef(
  (
    ...
    useImperativeHandle(ref, () => wrappingElementRef.current, [
      wrappingElementRef,
    ]);
    ...
    const enforceFocusHandler = useCallback(() => {
      ...
      const focusIsWithin = containers.some((m) =>
    ...
    useEffect(() => {
      ...
      const modalContainsFocus = modalElementRef.current?.contains(
      ...
      if (modalElementRef.current) {
        ...
        focusHandlerRef.current = listen(
          document,
          "focus",
          enforceFocusTimeout,
          true,
    ...
    return (
      <WrappingElementComponent ref={wrappingElementRef} {...rest}>
