...
type AvatarProps = { alt?: string; ariaLabel?: string; src?: string };

const Avatar = ({ src, alt, ariaLabel, ...rest }: AvatarProps) => {
  // Placeholder SVG
  let content = (
  ...
  if (src) {
    content = (
      <img src={src} alt={alt} aria-label={rest["aria-label"] || ariaLabel} />
    );
  }
  ...
  return (
    <div
      {...rest}
      className={classnames(css.avatar, rest.className)}
      data-test-avatar
    >
      {content}
    </div>
