...
const Popdown = ({
  ...
  const triggerHandleKeyDown = (e) => {
    triggerKeyHandler(e, open, disabled, toggleMenu, overlayRef, triggerRef, closeMenu);
  };

  const menuHandleKeyDown = (e) => {
    menuKeyHandler(e, open, disabled, toggleMenu, overlayRef, triggerRef, closeMenu);
  };

  const menuBlur = () => {
    /* timeout here is for handling toggle clicking of the dropdown trigger */
    focusTimeoutId = setTimeout(() => closeMenu(), 100);
  };
