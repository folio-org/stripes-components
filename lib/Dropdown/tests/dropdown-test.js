...
describe('Dropdown', () => {
  ...
  describe('usePortal prop', () => {
    const testFocusButton = Interactor('#testFocusButton');
    ...
    beforeEach(async () => {
      ...
      await mount(
        <React.Fragment>
          ...
          <button type="button" id="testFocusButton">testFocus</button>
        </React.Fragment>
      );
    });
    ...
    describe('clicking the trigger', () => {
      ...
      it('displays the dropdown menu', () => {
        expect(dropdown.isOpen).to.equal('true');
        expect(dropdown.items(0).isVisible).to.be.true;
      });

      it('focuses the first menu item', () => {
        expect(dropdown.items(0).isFocused).to.be.true;
      });

      describe('focusing the reverse proxy', () => {
        beforeEach(async () => {
          await dropdown.menu.focusStartProxy();
        });
      ...
      describe('focusing the forward proxy', () => {
        beforeEach(async () => {
          await dropdown.menu.focusEndProxy();
        });

        it('focuses the next tabbable item', () => {
          expect(testFocusButton.$root).to.equal(document.activeElement);
        });
      });
    });
  });
