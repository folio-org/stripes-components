...
class Dropdown extends React.Component {
  ...
  renderChildren(Tag, classes) {

    const { children, open, pullRight, disabled, id, ...attributes } = this.props;
    return React.Children.map(React.Children.toArray(children), (child) => {
      switch (child.props['data-role']) {
        case 'toggle': {
          const toggleChild = cloneElement(child, {
            open,
            pullRight,
  ...
  render() { 
    ...
    return(
      ...
      >
        {this.renderChildren(Tag,classes)}
      </TetherComponent>
     )
}
