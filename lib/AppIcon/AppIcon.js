...
const AppIcon = ({ iconType, iconSize, icon, style, active, children, className, tag, focusable }) => {
  /**
   * Formatted Icon
   ...
   */
  const getIcon = () => {
    /* Return null to show placeholder icon for icon style: default */
    if (!hasIcon && iconType === 'default') {
      return null;
    }

    /* Placeholder icon for icon style: popover */
    if (!hasIcon && iconType === 'popover') {
...
AppIcon.propTypes = {
  ...
  iconType: PropTypes.oneOf(['default', 'popover']),
