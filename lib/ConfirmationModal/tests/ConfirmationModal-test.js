...
describe('ConfirmationModal', () => {
  ...
  describe('when button labels are changed after mount', () => {
    const newConfirmLabel = 'New Confirm Label';
    const newCancelLabel = 'New Cancel Label';
    let updateLabels;
    ...
    beforeEach(async () => {
      await mountWithContext(<TestWrapper />);
    });

    it('should display the initial label for the confirm button', () => Button(confirmLabel).exists());

    it('should display the initial label for the cancel button', () => Button(cancelLabel).exists());

    describe('when updating the labels', () => {
      beforeEach(() => {
        updateLabels({
          cancel: newCancelLabel,
          confirm: newConfirmLabel,
        });
      });

      it('should display the new label for the confirm button', () => Button(newConfirmLabel).exists());

      it('should display the new label for the cancel button', () => Button(newCancelLabel).exists());
    });
  });
});
