...
import { getValueKey, getRegistryKey } from '../../../hooks/useCrossTabSessionStorage';
...
describe('SessionConfirmationModal', () => {
  ...
  describe('Checking the suppression checkbox', () => {
    ...
    it('should set the session key and call the confirm callback', () =>
      Promise.all([
        converge(() => {
          if (!sessionStorage.getItem(getValueKey(sessionKey))) { throw Error('Session key should be set'); }
        }),
  ...
  describe('and reopening the modal', () => {
    ...
    after(() => {
      sessionStorage.removeItem(getValueKey(sessionKey));
      localStorage.removeItem(getRegistryKey(sessionKey));
    });
