...
describe('AuditLogPane', () => {
  ...
  describe('should display loading pane if references are loading', () => {
    ...
    it('should render current version', () => HTML('Current version').exists());
  ...
  describe('when a modal window is open', () => {
    beforeEach(async () => {
      await mountWithContext(
        <RenderAuditLogPane
          versions={[mockVersions[0]]} // render only one version so there's just one "Changed" button
        />
      );
      await Button('Changed').click();
    });
