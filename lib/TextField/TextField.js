  ...
  getInputStyle() {
    ...
    if (this.props.meta) {
      validationClasses = classNames(
        ...
        { [`${css.textFieldWarning}`]: this.props.meta.touched && this.props.meta.warning },
...
console.warn('input props', this.props);
    ...
    const inputCustom = omitProps(cleanedProps, ['label', 'endControl', 'startControl', 'rounded', 'required', 'fullWidth', 'marginBottom0', 'noBorder', 'validationEnabled', 'meta']);
    console.log('inputProps', inputProps);
    console.log('inputCustom', inputCustom);
    const component = (
      <input
    ...
    if (this.props.meta) {
      if (validationEnabled && this.props.meta.asyncValidating && !this.props.meta.active) {
        validation = <div style={{ marginRight: '5px' }}><Icon title="validating" width="30px" size="small" icon="spinner-ellipsis" /></div>;
      }
      ...
      if (validationEnabled && this.props.meta.touched && this.props.meta.dirty && !this.props.meta.active && this.props.meta.valid && !this.props.meta.asyncValidating) {
        validation = <Icon iconClassName={css.successIcon} title="Field is valid" icon="validation-check" />;
      }
      if (this.props.meta.touched && !this.props.meta.active && !this.props.meta.valid && !this.props.meta.asyncValidating) {
        if (this.input && this.input.value === '' || this.input && !this.input.value) {
          validation = validationEnabled && <Icon iconClassName={css.errorIcon} title="Field has error" icon="validation-error" />;
        } else {
          validation = validationEnabled && <div style={{ display: 'flex', alignItems: 'center' }}><Icon iconClassName={css.errorIcon} title="Field has error" icon="validation-error" /></div>;
          clearField = <div style={{ display: 'flex', alignItems: 'center' }}><Button title="Clear this field" buttonStyle="fieldControl" onClick={this.clearField}><Icon icon="clearX" /></Button></div>;
        }
      }
      if (this.props.meta.active && this.props.input.value !== '') {
        clearField = <div style={{ display: 'flex', alignItems: 'center' }}><Button title="Clear this field" buttonStyle="fieldControl" onMouseDown={this.clearField} tabIndex="-1"><Icon icon="clearX" /></Button></div>;
      }
    }
