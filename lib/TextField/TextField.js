...
const TextFieldIcon = ({ onClick, title, icon, iconClassName, onMouseDown, tabIndex, iconSize }) => (
  <div style={{ display: 'flex', alignItems: 'center' }}>
    { typeof onClick === 'function' ? (
      <Button title={title} buttonStyle="fieldControl" onClick={onClick} onMouseDown={onMouseDown} tabIndex={tabIndex}>
  ...
  getInputStyle() {
    ...
    if (this.props.meta) {
      validationClasses = classNames(
        { [`${formStyles.hasFeedback}`]: this.props.meta.error || this.props.meta.warning },
        { [`${formStyles.feedbackWarning}`]: this.props.meta.touched && this.props.meta.warning },
        { [`${formStyles.feedbackError}`]: this.props.meta.touched && this.props.meta.error },
        { [`${formStyles.feedbackValid}`]: this.props.meta.touched && !this.props.meta.asyncValidating && this.props.meta.valid && this.props.meta.dirty },
      );
    }
...
console.warn('input props', this.props);
    ...
    const component = (
      <input
        ...
        className={classNames(formStyles.input, this.getInputStyle())}
        {...inputProps}
        {...inputCustom}
    ...
    if (this.props.meta) {
      ...
      if (this.props.meta.touched && !this.props.meta.active && !this.props.meta.valid && !this.props.meta.asyncValidating) {
        if (this.input && (this.input.value === '' || !this.input.value)) {
          ...
          clearField = <TextFieldIcon title="Clear this field" onClick={this.clearField} icon="clearX" />;
        }
      }
      if (this.props.meta.active && this.props.input.value !== '') {
        clearField = <TextFieldIcon title="Clear this field" onMouseDown={this.clearField} tabIndex="-1" icon="clearX" />;
      }
    }
