...
class TextField extends React.Component {
  ...
  componentDidUpdate(prevProps){
    ...
    if(prevProps.endControl !== this.props.endControl ||
      ...
      paddingObject = this.calcPadding();
      Object.assign(this.input.style, paddingObject);
    }
    ...
    if(this.props.meta){
      ...
      if(this.props.meta.touched && !this.props.meta.active && !this.props.meta.valid && !this.props.meta.asyncValidating){ 
        if(this.input.value === "" || !this.input.value){
          ...
          validation = <div style={{display:"flex", alignItems:"center"}}><Button title="Clear this field" buttonStyle="fieldControl" onClick={this.clearField}><Icon icon="closeX" /></Button><Icon title="Field has error" icon="validation-error" /></div>;
        }
      }
    }
    
    const endControlElement = <div className={css.endControls}><div className={css.controlGroup} ref={ref => this.endControl = ref}>{validation}{endControl}</div></div>;
