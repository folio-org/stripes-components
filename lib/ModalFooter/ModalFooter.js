...
const ModalFooter = ({ primaryButton, secondaryButton }) => {
  ...
  const renderButton = (buttonStyle, props) => {
    const { label, ...defaultButtonProps } = props; /* eslint-disable-line react/prop-types */
    return (<Button {...defaultButtonProps} buttonStyle={buttonStyle} marginBottom0>{label}</Button>);
  };
