...
const FocusLink = (props) => {
  ...
  if (component) {
    ...
    return (
      <Component ref={(ref) => { link = ref; }} role="button" tabIndex={tabIndex || 0} onClick={handleClick} onKeyPress={handleKeyPress} className={getClass()} {...rest}>
        { props.children }
      </Component>
    );
  }
  return (
    <div ref={(ref) => { link = ref; }} role="button" tabIndex={tabIndex || 0} onClick={handleClick} onKeyPress={handleKeyPress} className={getClass()} {...rest}>
      { props.children }
    </div>
  );
};
