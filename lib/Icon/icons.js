...
const icons = {
  ...
  'chevron-left': [
    <path d="M12.1 17.2l4 6.3c.7 1.1 2.1 1.4 3.2.7s1.4-2.1.7-3.2l-3.3-5 3.2-5.1c.7-1.1.4-2.5-.7-3.2-.4-.2-.8-.4-1.2-.4-.8 0-1.5.4-1.9 1.1l-4 6.3c-.5.8-.5 1.8 0 2.5z" />
  ],
  'chevron-right': [
    <path d="M19.9 14.8l-4-6.3c-.7-1.1-2.1-1.4-3.2-.7S11.3 9.9 12 11l3.2 5.1-3.2 5.1c-.7 1.1-.4 2.5.7 3.2.4.2.8.4 1.2.4.8 0 1.5-.4 1.9-1.1l4-6.3c.6-.9.6-1.9.1-2.6z" />
  ],
  'clock': [
  ...
  'spinner-ellipsis': ({ className, ...rest }) => (
    <div className={`${dotSpinner.spinner} ${className}`} {...rest} key="spinner-ellipsis">
...
/**
 * To avoid having duplicate code we loop over the icons list above
 * and build our SVG icon object which will return valid SVG's by defined keys
 *
 * Note: If the icon contents is not an array, we can assume that it's not returning SVG <path>'s
 * and we can just return it directly. An example of this is the "spinner-ellipsis" icon.
 */
export default Object.keys(icons).reduce((acc, key) => {
  const icon = icons[key];
  return Object.assign(acc, {
    [key]: props => (Array.isArray(icon) ? (
      <svg key={key} xmlns="http://www.w3.org/2000/svg" {...props}>
        {icon.map((path, i) => Object.assign({}, path, { key: `${key}-${i}` }))}
      </svg>
    ) : icon(props)),
  });
}, {});
