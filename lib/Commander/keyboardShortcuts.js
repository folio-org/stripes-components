...
/* function to import specific shortcuts relevant to the app. Takes in an array with
   ...
   shortcuts entries from the defaultKeyboardShortcuts array */
const importShortcuts = (shortcutNamesArray = []) => {
  return defaultKeyboardShortcuts
    .filter(({ name }) => shortcutNamesArray.find((shortcutName) => shortcutName?.toLowerCase() === name));
};
...
const renameShortcutLabels = (shortcutsArray = [], newLabelsArray = []) => {
  ...
  return shortcutsArray.map(shortcutObj => {
    ...
    const entryWithShortcut = newLabelsArray.find(({ shortcut }) => shortcut?.toLowerCase() === name);
