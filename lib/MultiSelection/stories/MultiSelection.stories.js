...
import { action } from '../../../node_modules/@bigtest/interactor';
...
const renderAddTag = ({ filterValue, exactMatch }) => {
  if (exactMatch) {
    ...
    return <div>Add tag for <strong>&quot;{`${filterValue}`}&quot;</strong></div>;
  }
};
    ...
    <div>
      <MultiSelection
        ...
        formatter={({ option, searchTerm }) =>
          (<OptionSegment searchTerm={searchTerm}>{option.value} {option.extra}</OptionSegment>)
        }
      />
    </div>
  ))
