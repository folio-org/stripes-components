...
type MultiSelectionProps = {
  ...
  dataOptions?: (string | Record<string, any>)[];
  dirty?: boolean;
  disabled?: boolean;
  emptyMessage?: string;
  error?: React.ReactNode;
  filter?: (...args: any[]) => any;
  filterProps?: { useContainsMatch?: boolean };
...
const filterOptions = (filterText, list, filterProps = {}) => {
  const { useContainsMatch = false } = filterProps;
  ...
  const pattern = useContainsMatch
    ? escapedFilterText
    : `^${escapedFilterText}`;
  const filterRegExp = new RegExp(pattern, "i");
  const renderedItems = filterText
    ? list.filter((item) => item.label.search(filterRegExp) !== -1)
    : list;
  const exactMatch = filterText
    ? renderedItems.filter((item) => item.label === filterText).length === 1
    : false;
  return { renderedItems, exactMatch };
};

const dbResize = debounce(
  (cb) => cb(window.matchMedia("(max-width: 640px)").matches),
...
const handleRemoval = (selectedItems, newSelectedItems, onRemove) => {
  if (selectedItems.length !== 0) {
    const removed = selectedItems.filter(
      (old) =>
        newSelectedItems.findIndex((newItem) => isEqual(old, newItem)) < 0,
    );
    if (removed.length > 0) {
      onRemove(removed[0]);
    }
...
const MultiSelection = ({
  ...
  const [atSmallMedia, setAtSmallMedia] = useState(
  ...
  const ariaLabelledBy = useRef(
    rest["aria-labelledby"] || rest.ariaLabelledby || rest.ariaLabelledBy,
  ).current;
  ...
  const dbFilter = useRef(
    debounce(filter, 300, { leading: false, trailing: true }),
  ).current;
  ...
  const options = useMemo(
    () =>
      asyncFiltering
        ? { renderedItems: dataOptions || [], exactMatch: false }
        : filter(filterValue, dataOptions, filterProps),
    [asyncFiltering, filterValue, dataOptions, filter, filterProps],
  );
  ...
  const {
    ...
    itemToKey:
      itemToString /* used by downshift's internal equality checker. We override it because it would default
  ...
  useEffect(() => {
    if (asyncFiltering) {
      dbFilter(filterValue, dataOptions, filterProps);
    }
  }, [filterValue, asyncFiltering, dataOptions, dbFilter, filterProps]);
  ...
  const onValueInputFocus = useCallback(() => {
    ...
    srStat.current.sendMessage(
      formatMessage({ id: "stripes-components.requiredFieldMessage" }),
    );
  ...
  */
  ...
  const renderOptions = useCallback(
    () =>
      options.renderedItems?.map((item, index) => (
        <SelectOption
  ...
  return (
    ...
    >
      ...
      <span className="sr-only" id={controlValueStatusId}>
        {`${selectedItems ? selectedItems.length : 0} item${
          !selectedItems || selectedItems.length !== 1 ? "s" : ""
        } selected`}
      </span>
      <span className="sr-only" id={controlDescriptionId}>
        {formatMessage({
          id: "stripes-components.multiSelection.controlDescription",
        })}
      </span>
      {/*
      ...
      <div className={css.multiSelectControlWrapper}>
        <div
          ...
          tabIndex="0"
          ref={control}
          aria-labelledby={`${labelProp ? labelId + " " : ""}${
            !labelProp && ariaLabelledBy ? ariaLabelledBy + " " : ""
        ...
        >
          <div className={css.multiSelectControlGroup}>
            ...
            {!atSmallMedia ? renderFilterInput() : <div>placeholder</div>}
            ...
            {...getToggleButtonProps({
              ...getDropdownProps({
                "aria-label": formatMessage({
                  id: "stripes-components.multiSelection.dropdownTriggerLabel",
                }),
...
export default formField(MultiSelection, ({ meta }) => ({
  dirty: meta.dirty,
