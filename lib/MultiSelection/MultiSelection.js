...
const MultiSelection= ({
  ...
  ...rest
    ...
    const sr_stat = useRef(null);
    const uiId = useRef(id || uniqueId('multiselect-')).current;
    ...
    const options = useMemo(
      ...
      [asyncFiltering, filterValue, dataOptions]
    );
    ...
    const {
      ...
      onStateChange({selectedItems: newSelectedItems, type}) {
        switch (type) {
          // this handles the variety of different ways that a selected option can be removed.
          ...
          case useMultipleSelection.stateChangeTypes
            ...
            const removed = selectedItems.filter(old => newSelectedItems.findIndex(newItem => isEqual(old, newItem)) === -1)
            if(removed !== -1) {
              onRemove(removed[0]);
            }
            break;
        ...
        switch (type) {
          ...
          case useCombobox.stateChangeTypes.ItemClick:
            if (newSelectedItem) {
              ...
              handleSelection(newSelectedItem);
            }
            break;
          default:
            break;
  ...
  // set up resize handler onMount...
    ...
    // for async filtering, call filter function as a debaounced after-effect of filter value changing...
    useEffect(() => {
      if (asyncFiltering) {
  ...
  const onValueInputFocus = useCallback(() => {
    atSmallMedia ? control.current?.focus() : input.current?.focus();
    sr_stat.current.sendMessage(formatMessage({ id: 'stripes-components.requiredFieldMessage' }));
  }, [atSmallMedia]);
    ...
    /* Filter input ends up rendered in one of two places. On smaller screens The filter input is placed in the overlay at the top
    ...
    */
    ...
    )), [options.renderedItems, selectedItems, highlightedIndex, filterValue]);
    ...
    const handleSelection = (selectedItem) => {
      if (selectedItems.findIndex((item) => isEqual(item, selectedItem)) !== -1) {
    ...
    return (
        <div className={css.multiSelectContainer} role="application" ref={container} id={uiId} aria-expanded={isOpen}>
          <SRStatus ref={sr_stat} />
          {label &&
            <Label
          ...
          */}
          <div className={css.multiSelectControlWrapper}>
            ...
            >
              <div className={css.multiSelectControlGroup}>
                ...
                {/* MultiSelectValueInput is visually hidden via CSS and contains string versions of selected items.
                    This allows for this component to be validated using HTML5 validation for the 'required' attribute.
                */}
                <input
            ...
            <MultiSelectResponsiveRenderer atSmallMedia={atSmallMedia} >
