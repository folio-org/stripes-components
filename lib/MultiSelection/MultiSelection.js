...
class MultiSelection extends React.Component {
  ...
  handleFilter = (filterValue) => {
    // const newState = this.props.filter(filterValue, this.props.dataOptions);
    const newState = { filterValue };
  ...
  handleDownshiftStateChange = (changes) => {
    if (Object.prototype.hasOwnProperty.call(changes, 'isOpen')) {
      if (changes.isOpen) {
        if (this.props.onOpenList) {
          this.props.onListOpen();
        }
      } else if (this.props.onCloseList) {
            ...
            return (
              <div className={css.multiSelectContainer} role="application">
                ...
                <span className="sr-only" id={controlValueStatusId}>
                  {`${selectedItems ? selectedItems.length : 0} item${selectedItems.length !== 1 ? 's' : ''} selected`}
                </span>
