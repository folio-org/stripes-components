...
const filterOptions = (filterText, list, useContainsMatch = false) => {
  ...
  const pattern = useContainsMatch ? escapedFilterText : `^${escapedFilterText}`;
...
const MultiSelection = ({
  ...
  const options = useMemo(
    () => (asyncFiltering ? { renderedItems: dataOptions || [], exactMatch: false } : filter(filterValue, dataOptions, useContainsMatch)),
    [asyncFiltering, filterValue, dataOptions, filter, useContainsMatch]
  );
  ...
  useEffect(() => {
    if (asyncFiltering) {
      dbFilter(filterValue, dataOptions, useContainsMatch);
    }
  }, [filterValue, asyncFiltering, dataOptions, dbFilter, useContainsMatch]);
...
const propTypes = {
  ...
  useContainsMatch: PropTypes.bool,
