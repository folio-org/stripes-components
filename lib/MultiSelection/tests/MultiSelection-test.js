...
const ValueChipInteractor = HTML.extend('value chip')
  ...
  const select = async (interactor, values) => {
    ...
    for (const value in valuesParam) {
      await MenuInteractor().find(OptionInteractor(value)).click();
    }
    await interactor.close();
  };

const LocalMultiSelection = MultiSelectInteractor.extend('local multiselection')
  .actions(
    {
      select
    }
