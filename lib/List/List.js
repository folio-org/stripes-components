...
const List = (props) => {
   ...
   */
  const noVisibleItems = () => (!props.items || !props.items.length) && !props.children;
...
console.log('props.marginBottom0', props.marginBottom0);
  /**
   * Get list classes
   */
   ...
   */
  if (noVisibleItems() && props.isEmptyMessage) {
    return (<p className={css.isEmptyMessage}>{props.isEmptyMessage}</p>);
  }

  return (
    <ul className={getListClass()}>
      {props.children || props.items.map(props.itemFormatter, this)}
    </ul>
  );
};
