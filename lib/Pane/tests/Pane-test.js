...
import { Pane as Interactor, Button as ButtonInteractor } from '@folio/stripes-testing';
...
describe('Pane', () => {
  ...
  describe('Dismissible pane', () => {
    ...
    const dismissPane = Interactor({ index: 1, id: 'dismissible' });
    let closed = false;
    ...
    describe('clicking the dismiss button', () => {
      ...
      // this only tests that the callback is fired
      // the second pane will also still exist at this point
      // we don't scaffold the state to control removing panes
  ...
  describe('Dismissible pane with internal buttons', () => {
    ...
    describe('clicking the dismiss button', () => {
      ...
      it('second pane is now the first', async () => {
        ...
        // the second pane will also still exist at this point
        // we don't scaffold the state to control removing panes
        // otherwise we could `await secondPane.absent();`
      });
    });
  });
  ...
  describe('content width, focus method coverage', () => {
    ...
    const button2 = ButtonInteractor('Content button 2');
    ...
    describe('focusing within a pane', () => {
      ...
      it('focuses input', () => button2.is({ focused: true }));
    });
  });
});
