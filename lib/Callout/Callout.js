...
class Callout extends React.Component {
  ...
  sendCallout({ type = 'success', message, timeout = 6000, dedupe = false }) {
    ...
    if (this._isMounted) {
      this.setState((curState) => {
        ...
        return { ...curState, callouts: { ...curState.callouts, [newCallout.id]: newCallout } };
      });
    }
  ...
  removeCallout(id) {
    ...
    if (this._isMounted) {
      this.setState((curState) => {
        const curCallouts = { ...curState.callouts };
        delete curCallouts[id];
        return { ...curState, callouts: curCallouts };
      });
    }
  }
