...
import { FIELD_NAMES, BOOLEAN_OPERATORS, MATCH_OPTIONS } from "../../constants";
...
type AdvancedSearchRowProps = {
  ...
  rowState: {
    "FIELD_NAMES.BOOL": string;
    "FIELD_NAMES.QUERY": string;
    "FIELD_NAMES.SEARCH_OPTION": string;
  };
...
const AdvancedSearchRow = ({
  ...
  const booleanOptions = useMemo(
    () =>
      Object.values(BOOLEAN_OPERATORS).map((boolOption) => ({
        id: boolOption,
        label: intl.formatMessage({
          id: `stripes-components.advancedSearch.boolean.${boolOption}`,
        }),
        value: boolOption,
      })),
    [intl],
  ...
  const matchOptions = useMemo(
    () =>
      Object.values(MATCH_OPTIONS).map((matchOption) => ({
        id: matchOption,
        label: intl.formatMessage({
          id: `stripes-components.advancedSearch.match.${matchOption}`,
        }),
        value: matchOption,
      })),
    [intl],
  ...
  return (
    <div className={styles.AdvancedSearchRow} data-row-index={index}>
      <Row>
        <Col xs={2}>
          {index === 0 ? (
            <b>
              <FormattedMessage id="stripes-components.advancedSearch.searchFor" />
            </b>
          ) : (
            <Select
              data-test-advanced-search-bool
              aria-label={intl.formatMessage({
                id: "stripes-components.advancedSearch.booleanLabel",
              })}
              onChange={(e) =>
                onChange(index, FIELD_NAMES.BOOL, e.target.value)
              }
        ...
        <Col xs>
          <TextArea
            ...
            aria-label={intl.formatMessage({
              id: "stripes-components.advancedSearch.fieldLabel",
            })}
        ...
        {hasMatchSelection && (
          <Col xs={3}>
            <Select
              ...
              aria-label={intl.formatMessage({
                id: "stripes-components.advancedSearch.match.label",
              })}
              onChange={(e) =>
                onChange(index, FIELD_NAMES.MATCH, e.target.value)
              }
        ...
        <Col xs={3}>
          <Select
            ...
            aria-label={intl.formatMessage({
              id: "stripes-components.advancedSearch.searchOptions.label",
            })}
            onChange={(e) =>
              onChange(index, FIELD_NAMES.SEARCH_OPTION, e.target.value)
            }
