...
import defaultSplitRow from '../../utilities/defaultSplitRow';
import {
  ROW_COUNT,
  FIELD_NAMES,
  BOOLEAN_OPERATORS,
...
const createInitialRowState = (firstRowInitialSearch, defaultSearchOptionValue) => {
  return new Array(ROW_COUNT).fill(null).map((_, index) => ({
    ...
    [FIELD_NAMES.MATCH]: MATCH_OPTIONS.EXACT_PHRASE,
  }));
};
...
const useAdvancedSearch = ({
  ...
  const initialRowState = useMemo(() => {
    ...
    return splitQueryRows(initialRows, splitRow);
  }, [firstRowInitialSearch, defaultSearchOptionValue, splitRow]);
  ...
  const filledRows = useMemo(() => filterFilledRows(splitQueryRows(rowState, splitRow)), [rowState, splitRow]);
  ...
  if (!isEqual(firstRowInitialSearch, prevFirstRowInitialSearch)) {
    const splitRows = splitQueryRows(initialRowState, splitRow);
