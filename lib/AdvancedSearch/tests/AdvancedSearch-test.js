...
describe('AdvancedSearch', () => {
  ...
  describe.only('when searching with query in first row', () => {
    beforeEach(async () => {
    ...
    it('should call onSearch with corectly formatted data', () => {
      expect(onSearchSpy.calledOnceWith('personalName==test or corporateConferenceName==test2', [
        { query: 'test', match: MATCH_OPTIONS.EXACT_PHRASE, searchOption: 'personalName', bool: '' },
        { query: 'test2', match: MATCH_OPTIONS.EXACT_PHRASE, searchOption: 'corporateConferenceName', bool: BOOLEAN_OPERATORS.OR },
      ])).to.be.true;
    });
  });
