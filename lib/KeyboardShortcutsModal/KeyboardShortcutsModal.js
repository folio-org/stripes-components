...
function KeyboardShortcutsModal(props) {
  ...
  // deduplicate core shortcuts from the allCommands array
  const deDupedShortcutsArray = uniqBy([...allCommands, ...coreShortcuts], 'name');

  const shortcutsData = deDupedShortcutsArray.map(({ label, shortcut }) => {
