...
class FieldRow extends React.Component {
    ...
    return (
      <Layout className={index === 0 ? 'marginTopLabelSpacer' : ''} >
        <Button
          ...
          title={this.props.intl.formatMessage({ id: 'stripes-components.removeFields' }, { item: this.props.label, num: index + 1 })}
        >
          <Icon icon="trashBin" />
        </Button>
      </Layout>
  ...
  render() {
    const { fields } = this.props;
    let addLabel = this.props.intl.formatMessage({ id: 'stripes-components.addNewField' }, { item: this.props.label });
