  ...
  render() {
    ...
    const addressBlocks = filteredAddresses.map(
      (ad, i) => {
        ...
        {
          return (<AddressEdit key={ad[uniqueField]} headerComponent={headerFormatter ? headerFormatter.edit : undefined } addressObject={ad} handleCancel={this.handleCancelEdit} handleDelete={this.handleDeleteEdit} uiId={ad[uniqueField]} initialValues={ad} onSubmit={this.handleSubmitEdit} canDelete={canDelete} {...viewProps}/>);
        }
      }
    );
    const newAddressBlocks = this.state.newAddresses.map(
      (ad) => {
        return (<AddressEdit key={ad} addressObject={ad} headerComponent={headerFormatter ? headerFormatter.edit : undefined } handleCancel={this.handleCancelNew} uiId={ad} initialValues={{guiid:ad}} onSubmit={this.handleSubmitNew} prevFocused={document.activeElement} newAddress/>);
      }
    );
