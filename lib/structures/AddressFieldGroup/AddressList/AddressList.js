...
class AddressList extends React.Component {
  ...
  render() {
    ...
    const newAddressBlocks = this.state.newAddresses.map(ad =>
      (<AddressEdit form={`addressForm-new-${ad}`} key={ad} addressObject={{ id: ad }} headerComponent={headerFormatter ? headerFormatter.edit : undefined} handleCancel={this.handleCancelNew} uiId={ad} initialValues={{ guiid: ad }} onSubmit={this.handleSubmitNew} prevFocused={document.activeElement} {...viewProps} />),
    );
    ...
    return (
      <div aria-label="Address Section" tabIndex="0" role="tabpanel" ref={(ref) => { this.container = ref; }}>
        <Row>
          ...
          <Col xs={4}>
            <Layout className="right"><Button onClick={this.handleAddNew}>+ Add another address</Button></Layout>
          </Col>
        </Row>
