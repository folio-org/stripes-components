  ...
  render() {
    ...
    const addressBlocks = filteredAddresses.map((ad) => {
      if (this.state.editting.indexOf(ad[uniqueField]) === -1) {
       ...
       console.log('editing address', ad);
    ...
    const newAddressBlocks = this.state.newAddresses.map((ad) => {
      console.log('adding new address', ad);
      return (<AddressEdit form={`addressForm-new-${ad}`} key={ad} addressObject={{ id: ad }} headerComponent={headerFormatter ? headerFormatter.edit : undefined} handleCancel={this.handleCancelNew} uiId={ad} initialValues={{ guiid: ad }} onSubmit={this.handleSubmitNew} prevFocused={document.activeElement} />);
    });
