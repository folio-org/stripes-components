...
const AddressEdit = (props) => {
  ...
  visibleFields.forEach((field, i) => {
    const fieldLabel = labelMap.hasOwnProperty(field) ? labelMap[field] : field;
    const fieldComponent = (<Col key={`col-${i}`} lxs={4}><Field label={fieldLabel} name={field} component={mergedFieldComponents[field]} /></Col>);
