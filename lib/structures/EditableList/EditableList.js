...
class EditableList extends React.Component {
  ...
  CreateItemFormatter = (item) => {
    ...
    if (this.state.creatingArray.length === 1) {
      ...
      editingIndex = this.getItemIndex(id, this.state.creatingArray);
    }
    ...
    visibleFields.forEach(
      (field) => {
        const fieldContent = (<TextField
          ...
          name={field}
          onFocus={e => this.handleCreateFieldFocus(e, id)}
          fullWidth
        />);
        const renderedField = (
    ...
    const actions = (<div style={{ float: 'right' }}>
      <button onClick={e => this.handleCreateCancelClick(e, id)}>Cancel</button>
      <button onClick={e => this.handleCreateSave(e, id)}>Save</button>
    </div>);
    ...
    return (
      <li
        ...
        ref={id === this.state.tempItem[uniqueField] ?
  ...
  handleEditCancelClick(e, id) {
    e.preventDefault();

    const ind = (this.state.editArray.length === 1)
      ? 0
  ...
  EditItemFormatter = (item) => {
    ...
    const editingIndex = this.getItemIndex(id, this.state.editArray);

    const isEditing = editingIndex !== -1;
    const editItem = this.state.editArray[editingIndex];
    const fields = visibleFields.filter((field) => field !== uniqueField);

    let row = (<div></div>);

    if (isEditing) {
      row = (<EditableItemForm
        id={id}
        formId={`form-${id}`}
        initialValues={this.getInitialValues(editItem, fields)}
        onChange={this.handleFieldChange}
        onCancelClick={this.handleEditCancelClick}
        handleSaveClick={this.handleSaveEditClick}
        handleSubmit={this.handleSaveEditClick}
      />);
     ...
     fieldContent = (<TextField
              ...
              placeholder={field}
              name={field}
              onFocus={e => this.handleFieldFocus(e, id)}
              fullWidth
              marginBottom0
            />)
      */
      ...
      const renderedData = fields.map(field =>
        ...
        <Row end="xs" around="xs">
          <Col xs>
            { !this.actionSuppression.edit(item) &&
              <Button bottomMargin0 onClick={e => this.handleEditClick(e, id)} aria-label="Edit Item">Edit</Button>
            }
            { this.actionSuppression.delete(item) &&
              <Button bottomMargin0 onClick={e => this.handleDeleteClick(e, id)} aria-label="Delete Item">Delete</Button>
            }
          </Col>
        </Row>
      );
    ...
    if (isEditing) {
      ...
      actions = (<div style={{ float: 'right' }}>
        <button onClick={e => this.handleEditCancelClick(e, id)}>Cancel</button>
        <button onClick={e => this.handleSaveEditClick(e, id)}>Save</button>
      </div>);
    } else {
      ...
      actions = (
        <div style={{ float: 'right' }}>
          { !this.actionSuppression.edit(item) &&
            <button onClick={e => this.handleEditClick(e, id)} aria-label="Edit Item">Edit</button>
          }
          { this.actionSuppression.delete(item) &&
            <button onClick={e => this.handleDeleteClick(e, id)} aria-label="Delete Item">Delete</button>
          }
        </div>);
    }
    */
    ...
    return (
      <div>
        ...
        <Row>
          <Col xs={12}>
            <List
              items={this.state.creatingArray}
