...
const ItemEdit = ({ rowIndex, error, field, visibleFields, fieldComponents, readOnlyFields, widths, cells, autoFocus }) => {
        ...
        return (
          <div key={`${field}-${name}-${rowIndex}`} style={{ flex: `0 0 ${widths[name]}`, width: `${widths[name]}`, padding: '6px' }}>
            <Field
              ...
              component={fieldComponents[name].component}
              fullWidth
              marginBottom0
              {...fieldComponents[name].props}
            />
          </div>
        );
      }
