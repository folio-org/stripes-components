...
class EditableListForm extends React.Component {
  constructor(props) {
    ...
    if (props.initialValues) {
      status = props.initialValues.items.map(() => ({ editing: false, error: false }));
    }
    ...
    this.testingId = this.props.label.replace(/\s/g, '').toLowerCase();
  }

  componentWillReceiveProps(nextProps) {
    if (!isEqual(this.props.initialValues, nextProps.initialValues)) {
      this.setState({
        status: nextProps.initialValues.items.map(() => ({ editing: false, error: false })),
      });
    }
  }

  buildStatusArray() {
    return this.props.initialValues.items.map(() => ({ editing: false, error: false }));
  }
      ...
      <div>
        <Row between="xs" className={css.editableListFormHeader}>
          ...
          <Col xs>
            <Row end="xs">
              <Col xs>
                <Button onClick={() => this.onAdd(fields)} id={this.props.addButtonId}>
