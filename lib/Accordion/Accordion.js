...
// if focus is within the accordion, return the highest z-index of the set.
// otherwise, return the z-index according to registration.
function getStackOrder(contentRef, accordionSet, stackOrder) {
  if (contentRef?.matches(':focus-within')) {
    return accordionSet?.getNumberOfAccordions() || 2;
  }
  return stackOrder;
}
...
const Accordion = (props) => {
  ...
  return (
    ...
    >
      ...
      <div className={getWrapClass(isOpen)} style={{ zIndex: getStackOrder(content.current, accordionSet, stackOrder) }}>
        <div
          role="region"
