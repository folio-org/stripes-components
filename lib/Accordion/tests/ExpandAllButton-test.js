...
describe('ExpandAll button', () => {
  ...
  describe('when accordions are open', () => {
    ...
    it('should exist', () => {
      converge(() => expect(expandAllButton.hasButton).to.be.true);
    });

    it('should have id', () => {
      converge(() => expect(expandAllButton.id).to.equal('expand-button'));
    });

    it('button label should be equals', () => {
      converge(() => expect(expandAllButton.label).to.equal(translations.collapseAll));
    });
