...
type AccordionSetProps = {
  accordionStatus?: Record<string, any>;
  children?: React.ReactNode | React.ReactNode[];
  id?: string;
  inheritStatus?: boolean;
  initialStatus?: Record<string, any>;
  onRegisterAccordion?: (...args: any[]) => any;
  onToggle?: (...args: any[]) => any;
  onUnregisterAccordion?: (...args: any[]) => any;
  setStatus?: (...args: any[]) => any;
};

class AccordionSet extends React.Component<AccordionSetProps> {
  static defaultProps = {
    inheritStatus: true,
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      Object.keys(prevState.status).length !==
      Object.keys(this.state.status).length
    ) {
      if (this.innerStatus.current)
        this.innerStatus.current.setStatus(this.state.status);
    }
  }

  focusNextAccordion(e) {
    const currentAccordionIndex = this.state.accordionList.findIndex((a) =>
      e.target.id.includes(a.id),
    );
  ...
  focusPreviousAccordion(e) {
    ...
    if (currentAccordionIndex !== -1) {
      if (currentAccordionIndex > 0) {
        const prevRef =
          this.state.accordionList[currentAccordionIndex - 1].getRef();
        prevRef.focus();
  ...
  unregisterAccordion = (id) => {
    const accordionIndex = this.state.accordionList.findIndex(
      ...
      <StatusContext>
        ...
        >
          {({ status, onToggle }) => (
            ...
            >
              <div id={id}>
                <StripesOverlayWrapper>{children}</StripesOverlayWrapper>
              </div>
            </AccProvider>
          )}
        </AccordionStatus>
