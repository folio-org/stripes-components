...
const DefaultAccordionHeader = (props) => {
  ...
  function headerContentRight() {
    const { open, displayWhenOpen, displayWhenClosed } = props;
    const content = open ? displayWhenOpen : displayWhenClosed;
    if (!content) {
      return null;
    }
    return (<div className={css.headerDefaultContentRight}>{content}</div>);
  }
