...
const FilterAccordionHeader = forwardRef(
  (
    ...
    const clearButtonVisible =
      displayClearButton && typeof onClearFilter === "function";
    const labelPropsId = label?.props?.id;
    const labelText = labelPropsId
      ? formatMessage({ id: labelPropsId })
      : label || "-";
      ...
      <div className={css.headerWrapper} ref={ref}>
        ...
        >
          <button
            ...
            aria-expanded={open}
            disabled={disabled}
            className={classNames(css.filterSetHeader, {
              [css.clearButtonVisible]: clearButtonVisible,
            })}
            autoFocus={autoFocus}
            ref={toggleRef}
            id={labelId}
