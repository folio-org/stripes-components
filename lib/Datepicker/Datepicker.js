...
class Datepicker extends React.Component {
  constructor(props) {
    ...
    if (!this.props.input) {
      ...
      inputValue = moment(this.props.input.value).format(this.props.dateFormat);
    }
  ...
  handleKeyDown(e) {
    if (this.picker) {
      ...
      const formattedDate = curDate.format(this.props.dateFormat);
      let dateString;
      switch (e.keyCode) {
        case 40: // down
          e.preventDefault();
  ...
  handleSetDate(e, date, stringDate) {
    if (e.type === 'click' || e.type === 'keydown') {
      ...
      if (this.props.input) {
        if (moment(stringDate, this.props.dateFormat, true).isValid()) {
    ...
    if (e.type === 'change') {
      ...
      if (this.props.onChange) { this.props.onChange(e); }
      if (this.props.input) {
        if (moment(e.target.value, this.props.dateFormat, true).isValid()) {
          // convert date to ISO 8601 format for backend
