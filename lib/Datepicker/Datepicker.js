...
const Datepicker = (
  ...
  const datePickerIsFocused = () => {
    if (contains(container.current, document.activeElement) &&
      ...
      if (calendar.current) {
        return (contains(calendar.current, document.activeElement));
      }
      return true;
    }
    return false;
  ...
  const handleRootClose = (e) => {
    if (!contains(container.current, e.target) || !contains(calendar.current, e.target)) {
  ...
  const content = (
    <div className={css.container} ref={container}>
      <TextField
        ...
        onBlur={(e) => { e.preventDefault(); props.onBlur(e); }}
        hasClearIcon={false}
        inputRef={input}
        placeholder={format}
      />
