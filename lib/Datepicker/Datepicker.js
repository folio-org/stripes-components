...
const defaultParser = (value, timeZone, uiFormat, outputFormats) => {
  ...
  if (containsUTCOffset(value)) {
    inputMoment = timeZone ? dayjs(value).tz(timeZone) : dayjs(value);
  } else {
    inputMoment = dayjs(value, [uiFormat, ...outputFormats]).tz('utc');
  }
...
export const defaultOutputFormatter = ({ backendDateStandard, value, uiFormat, outputFormats, timeZone }) => {
  ...
  let parsed;
  if (containsUTCOffset(value)) {
    parsed = timeZone && timeZone.toLowerCase() !== 'utc' ? dayjs(value).tz(timeZone) : dayjs(value);
  } else {
    parsed = dayjs(value, [uiFormat, ...outputFormats]);
  }
