...
const Datepicker = (
  ...
  open.current = showCalendar;
  const input = useRef(null);
  const pickerRef = useRef(null);
  const blurTimeout = useRef(null);
  const hiddenInput = useRef(null);
  const testId = useRef(id || uniqueId('dp-')).current;
  const calendarfirstButton = useRef(null);
  const container = useRef(null);
  const payload = useRef(datePair);

  const whenCalendarOpen = useRef(() => {
    calendarfirstButton.current?.focus();
  }).current;

  const datePickerIsFocused = useRef(() => {
  ...
  const handleClickOutside = useRef((e) => {
    if (open.current && !(container.current?.contains(e.target) || pickerRef.current?.contains(e.target))) {
      if (!datePickerIsFocused.current()) {
        setShowCalendar(false);
        // console.log('closed on root close');
      }
    }
  });
  ...
  useEffect(() => {
    if (showCalendar) {
      document.addEventListener('click', handleClickOutside.current, true);
      setTimeout(whenCalendarOpen);
    } else {
      document.removeEventListener('click', handleClickOutside.current, true);
    }
  }, [showCalendar]);
  ...
  const handleRootClose = (e) => {
    // if (open && (!contains(container.current, e.target) || !contains(pickerRef.current, e.target))) {
  ...
  const renderCalendar = () => (
    <Calendar
      ...
      dateFormat={format}
      firstButtonRef={calendarfirstButton}
  ...
  return (
    <div className={css.container} {...pickDataProps(props)}>
      ...
      <Popper
        ...
        portal={portalElem}
        whenOpen={whenCalendarOpen}
        overlayProps={
          {
            "className": css.calendarOverlay,
            "data-test-calendar-overlay": true,
          }
        }
        modifiers={{
          offset: {
