...
const getLocalFormat = ({ // eslint-disable-line object-curly-newline
  // intlLocale,
  // localeProp,
  intl }) => {
...
const defaultParser = (value, timeZone, uiFormat, outputFormat) => {
  ...
  if (offsetRegex.test(value)) {
    ...
    inputMoment = moment.tz(value, [uiFormat, outputFormat], timeZone);
  }
...
const defaultOutputFormatter = ({ backendDateStandard, value, uiFormat, outputFormat, timeZone }) => {
  const DATE_RFC2822 = 'ddd, MM MMM YYYY HH:mm:ss ZZ';
  const parsed = new moment.tz(value, [uiFormat, outputFormat], timeZone); // eslint-disable-line 
...
const Datepicker = (
  ...
  const [dateString, updateDateString] = useState(
    valueProp ? parser(
      ...
      timeZone, // timezone
      format, // uiFormat
      'YYYY-MM-DDTHH:mm:ss.sssZ' // outputFormat
    ) : ''
  );

  const [formatted, updateFormatted] = useState(
    valueProp ? outputFormatter({
      valueProp,
      timeZone,
      uiFormat: format,
  ...
  const [internalValueChanged, updateInternalValueChanged] = useState(false);
  ...
  useEffect(() => {
    if (useInput && formatted !== null) {
      nativeChangeField(hiddenInput, false, formatted);
      updateInternalValueChanged(false);
    }
  }, [useInput, formatted]);
  ...
  const maybeUpdateValue = (value) => {
    ...
    if (isValid || value === '') {
      ...
      if (parsed !== dateString) {
        updateInternalValueChanged(true);
        updateDateString(parsed);
        const hiddenDate = outputFormatter({
          backendDateStandard,
          value: parsed,
          uiFormat: format,
          outputFormat: backendStandard,
          timeZone
        });
        updateFormatted(hiddenDate);
      }
  ...
  if (typeof valueProp !== 'undefined' && !internalValueChanged) {
  ...
  const onChangeFormatted = (e) => {
    // updateFormatted(e.target.value);
    if (useInput && outputBackendValue && onChange) {
  ...
  const content = (
    <div className={css.container} ref={container}>
      <TextField
        ...
        placeholder={format}
      />
      <input type="text" hidden onChange={onChangeFormatted} ref={hiddenInput} />
    </div>
  );
