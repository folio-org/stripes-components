...
const Datepicker = ({
  ...
  const [dateString, updateDateString] = useState(
    parser({ valueProp, timeZone: 'utc', dateFormat: 'MM/DD/YYYY' }) || ''
  );
  ...
  const maybeUpdateValue = (value) => {
    ...
    if (isValid || value === '') {
      const parsed = parser({ value, timeZone: 'utc', dateFormat: 'MM/DD/YYYY' });
  ...
  // if calendar is open, focus can be moved to it by pressing the down arrow...
  // if calendar is closed, down arrow will open it.
  ...
  const content = (
    <div className={css.container} ref={container}>
      <TextField
        ...
        onFocus={useFocus && (() => setShowCalendar(true))}
