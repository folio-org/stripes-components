...
const defaultOutputFormatter = ({ backendDateStandard, value, uiFormat, outputFormat, timeZone }) => {
  ...
  return moment(normalizedDate, 'YYYY-MM-DD').format(backendDateStandard);
};
...
const Datepicker = (
  ...
  useEffect(() => {
    if (typeof valueProp !== 'undefined' && valueProp !== datePair.dateString && valueProp !== datePair.formatted) {
      ...
      nativeChangeField(input, true, payload.current.dateString);
    }
  ...
  maybeUpdateValue = (value) => {
    ...
    if (isValid) {
      ...
      if (parsed !== datePair.dateString) {
        ...
        return { dateString: parsed, formatted: hiddenDate };
      }
      return {};
      ...
      updateDatePair(current => Object.assign(current, dates));
      return dates;
    }
    return {};
  };
