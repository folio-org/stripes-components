...
class DatepickerDemo extends React.Component {
  ...
  render() {
    return (
      <div>
        <div>State Value 1: {this.state.day0}</div>
        <div>State Value 2: {this.state.day1}</div>
        <div>State Value 3: {this.state.day2}</div>
        <div>State Value 4: {this.state.rfd1}</div>
        <div>State Value 5: {this.state.day3}</div>
        <button onClick={this.clearDate} type="button">Clear dates</button>
        <Datepicker
          label="Date - calendar defaults to displaying the current month if no date is selected."
          locale="en-SE"
          value={this.state.day0}
          onChange={(e) => { this.onChangeDate(e.target.value, 'day0'); }}
        />
        <Datepicker
          label="Date"
          locale="en-SE"
