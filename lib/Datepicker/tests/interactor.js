...
@interactor class Calendar {
  ...
  monthField = new MonthField('[data-test-month-select]');
  yearField = new YearField('[data-test-year-input]');
  ...
  focusCalendarDay = focusable('button[data-day][tabindex="0"]');
  ...
  get header() {
    return `${monthArray[this.monthField.value]} ${this.yearField.value}`;
  }
}
...
export default interactor(class DatepickerInteractor {
  ...
  pressEnter = triggerable('button[data-day][tabindex="0"]', 'keydown', {
    bubbles: true,
    cancelable: true,
    keyCode: 13
  });
  ...
  pressEscape = triggerable('button[data-day][tabindex="0"]', 'keydown', {
    bubbles: true,
    cancelable: true,
    keyCode: 27
  });
