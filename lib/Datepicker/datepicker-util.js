...
/** defaultValidator
 *  validates user input to determine whether the value is output.
...
// passes invalid input directly through, leaving validation up
// to the consuming app.
export const passThroughOutputFormatter = ({ backendDateStandard, value, uiFormat, outputFormats, timeZone }) => {
  if (!value || value === '') { return value; }
  if (defaultInputValidator({ value, format: uiFormat, backendStandard: [] })) {
