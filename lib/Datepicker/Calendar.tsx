...
type CalendarProps = {
  ...
  fillParent?: boolean;
  firstFieldRef?: Record<string, any> | ((...args: any[]) => any);
  id?: string;
  ...
  onSetDate?: (...args: any[]) => any;
  rootRef?: ((...args: any[]) => any) | { current?: Element };
  selectedDate?: Record<string, any> | string;
  trapFocus?: boolean;
};
...
function getMonths(intl) {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    months.push(
      intl.formatDateToParts(`${monthString}/13/${new Date().getFullYear()}`, {
        month: "long",
        timeZone: "UTC",
      })[0].value,
    );
...
function getWeekDays(intl) {
  ...
  for (let i = 15; i <= 22; i++) {
    weekDays.push(
      intl.formatDateToParts(new Date(Date.UTC(2020, 2, i)), {
        weekday: "short",
        timeZone: "UTC",
      })[0].value,
    );
...
class Calendar extends React.Component<CalendarProps> {
  constructor(props) {
    ...
    if (!selectedDate) {
      ...
      // no pre-selected date, datestring invalid, init as 'today'.
      cursorDate = new moment(); // eslint-disable-line new-cap
    }
    ...
    this.calendarLabel = props.intl.formatMessage({
      id: "stripes-components.Datepicker.calendar",
    });
    this.calendarRoledescription = props.intl.formatMessage({
      id: "stripes-components.Datepicker.calenderRole",
    });
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (nextProps.selectedDate !== prevState.selectedDate) {
      ...
      if (moDate.isValid()) {
        ...
        // fix navigation issue on null or invalid date
  ...
  updateCursorDate = (date) => {
    this.setState(
      (oldState) => {
        ...
        if (
          newState.year !== oldState.year ||
          newState.month !== oldState.month
        ) {
          newState.calendar = getCalendar(
            newState.year,
            newState.month,
            oldState.dayOffset,
          );
      ...
      () => {
        ...
        const nextButtonElem = document.getElementById(
          `datepicker-choose-date-button-${cursorString}-${id}`,
        );
  ...
  render() {
    ...
    const weeks = this.state.calendar.map((week) => {
      weekCount += 1;
      const dayList = [];
      ...
      const days = dayList.map((day) => {
        ...
        const excludedAria = isExcluded
          ? intl.formatMessage({
              id: "stripes-components.Datepicker.unavailable",
            }) + " "
          : "";
    ...
    return (
      ...
      >
        ...
        {trapFocus && <div onFocus={this.focusTrap.toLast} tabIndex="0" />}
        <div className={css.calendarInner}>
          <div className={css.calendarHeader} data-test-calendar-header>
          ...
          <ul className={css.daysOfWeek}>{daysOfWeek}</ul>
          <FormattedMessage id="stripes-components.Datepicker.calendarDaysListDescription">
            {([description]) => (
              <div className="sr-only" id={`${id}-cal-description`}>
                {description}
              </div>
            )}
          </FormattedMessage>
