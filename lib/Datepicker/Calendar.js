...
class Calendar extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.selectedDate !== this.props.selectedDate) {
      const moDate = moment(nextProps.selectedDate, this.props.dateFormat, true);
      if (moDate.isValid()) {
        if (moDate !== this.state.date) {
  ...
  render() {
            ...
            return (
              <td key={day.format('D-MM')} >
                <DayButton
                  ...
                  title={formatMsg({ id: 'stripes-components.selectDay' }, { dayOfWeek: titleDayString, date: titleFormattedDay })}
    ...
    return (
      <div className={css.calendar} id={`datepicker-calendar-container-${id}`} ref={(r) => { this._calendarContainer = r; }}>
        <div className={css.calendarInner}>
          <table role="presentation">
            <thead>
              <tr>
                <td>
                  <button id={`datepicker-previous-year-button-${id}`} className={css.nav} type="button" onClick={this.previousYear} onKeyDown={this.props.onKeyDown} tabIndex="-1" style={arrowStyle} title={formatMsg({ id: 'stripes-components.goToPreviousYear' })}><Icon icon="left-double-chevron" /></button>
                </td>
                <td>
                  <button id={`datepicker-previous-month-button-${id}`} href="#" className={css.nav} type="button" onClick={this.previousMonth} onKeyDown={this.props.onKeyDown} tabIndex="-1" style={arrowStyle} title={formatMsg({ id: 'stripes-components.goToPreviousMonth' })}><Icon icon="left-arrow" /></button>
                </td>
                <td colSpan="3"><span className={css.selectedMonth}>{moment().month(this.state.month).format('MMMM')} {this.state.year}</span></td>
                <td>
                  <button href="#" id={`datepicker-next-month-button-${id}`} className={css.nav} type="button" onClick={this.nextMonth} onKeyDown={this.props.onKeyDown} tabIndex="-1" style={arrowStyle} title={formatMsg({ id: 'stripes-components.goToNextMonth' })} ><Icon icon="right-arrow" /></button>
                </td>
                <td>
                  <button href="#" id={`datepicker-next-year-button-${id}`} className={css.nav} type="button" onClick={this.nextYear} onKeyDown={this.props.onKeyDown} tabIndex="-1" style={arrowStyle} title={formatMsg({ id: 'stripes-components.goToNextYear' })}><Icon icon="right-double-chevron" /></button>
                </td>
              </tr>
              <tr>
                {daysOfWeek}
