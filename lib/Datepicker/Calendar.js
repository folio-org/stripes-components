...
function getWeekDays(intl) {
  ...
  for (let i = 15; i <= 22; i++) {
    weekDays.push(
      intl.formatDateToParts(new Date(Date.UTC(2020, 2, i)), { weekday: 'short', timeZone: 'UTC' })[0].value
    );
  }
  return weekDays;
}
...
class Calendar extends React.Component {
  constructor(props) {
    ...
    // if the stripes locale has no region (only 2 letters), it needs to be normalized to a common form of language-region
    // we adjust it by mapping from a set of common default regions (staticRegions);
    // the first weekday in calendar rendering is derived from region. Hopefully it will be implemented in browser Intl API soon..
    // but until then...
    let dayOffset = 0;
