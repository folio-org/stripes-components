...
function getCalendar(year, month, offset = 0) {
  ...
  const firstDay = dayjs(startDate).startOf('month');
  const endDay = dayjs(startDate).endOf('month');
...
class Calendar extends React.Component {
  constructor(props) {
    ...
    this.selectedDate = new dayjs(); // eslint-disable-line new-cap
    let cursorDate;
    if (!selectedDate) {
      cursorDate = new dayjs(); // eslint-disable-line new-cap
    } else if (dayjs(selectedDate, dateFormat, true).isValid()) {
      this.selectedDate = new dayjs(selectedDate, dateFormat, true); // eslint-disable-line new-cap
    ...
    const base = new dayjs(cursorDate); // eslint-disable-line new-cap
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (nextProps.selectedDate !== prevState.selectedDate) {
      const moDate = new dayjs(nextProps.selectedDate, nextProps.dateFormat, true); // eslint-disable-line new-cap
      if (moDate.isValid()) {
        ...
        const fallbackDate = new dayjs(); // eslint-disable-line new-cap
  ...
  isDateSelected = (day) => {
    if (this.props.selectedDate) {
      return day.format(this.props.dateFormat) ===
        new dayjs(this.props.selectedDate, this.props.dateFormat, true) // eslint-disable-line new-cap
          .format(this.props.dateFormat);
    }
    if (this.state.date) {
      return day.format(this.props.dateFormat) ===
        new dayjs(this.state.date, this.props.dateFormat, true) // eslint-disable-line new-cap
          .format(this.props.dateFormat);
    }
    return false;
  }
