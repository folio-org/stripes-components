...
import { dayjs, DayRange } from '../../util/dateTimeUtils';
...
function getCalendar(year, month, offset = 0) {
  ...
  const monthRange = new DayRange(firstDay, endDay).expand();
  ...
  for (let i = 0; i < weeks.length; i += 1) {
    const weekRange = new DayRange(rowStartArray[i], rowEndArray[i]).expand();
