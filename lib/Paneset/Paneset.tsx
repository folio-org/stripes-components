...
class Paneset extends React.Component<PanesetProps> {
  constructor(props) {
    super(props);
  ...
  resizeToContainer = () => {
    ...
    // the "paneId" array.
    const matchingCacheIndex =
      this.state.layoutCache?.findIndex((c) => {
    ...
    if (matchingCacheIndex !== -1) {
        ...
        if (Math.floor(totalWidth) !== currentWidth) {
          ...
          for (const p in matchingLCache) {
            if (Object.prototype.hasOwnProperty.call(matchingLCache, p)) {
              const newPO = getNewPanewidthObject(
                p,
                matchingLCache[p],
                proportionalChange,
              );
          ...
          this.setState(
            { changeType: "windowResizeNestedPanesetPanes" }, // eslint-disable-line react/no-unused-state
            () => {
              this.resizeNestedTO = setTimeout(this.resizeNestedPanesets);
            },
          );
      ...
      if (Math.floor(totalWidth) !== currentWidth) {
        ...
        const newWidths = this.calcWidths(tempPanesList);
        this.setState(
          { changeType: "windowResizeUncached" }, // eslint-disable-line react/no-unused-state
          () => {
            this.resizePanes(this.state.panes, newWidths);
            this.resizeNestedTO = setTimeout(this.resizeNestedPanesets);
          },
        ...
        // nested panesets may need resizing...
        this.setState(
          { changeType: "windowResizeNestedUncached" }, // eslint-disable-line react/no-unused-state
          () => {
            this.resizeNestedTO = setTimeout(this.resizeNestedPanesets);
          },
        );
  ...
  handleWindowResize = () => {
    const { isRoot, paneset } = this.props;
    if (isRoot || !paneset) {
      ...
      if (paneset && !isRoot) {
        ...
        if (containerBounds.right < window.offsetWidth) {
          this.setState((curState) => {
            const { left, right } = containerBounds;
    ...
    this.setState(
      (oldState) => {
        ...
        if (paneObject.transition !== "none") {
          ...
          const paneExists = newState.panes.findIndex(
            (p) => p.id === paneObject.id,
          );
          if (paneExists === -1) {
  ...
  resizeCachedSizesToFit = (layoutIndex) => {
    ...
    if (!paneset) {
      ...
      for (const p in layoutObject) {
        if (layoutObject[p]) {
          ...
          if (unit === "px") {
            ...
            value =
              (parseFloat(layoutObject[p], 10) / 100) *
      ...
      if (
        Object.values(layoutUnitTypes).every(
          (v) => v === "percent" || v === "vw",
        )
      ) {
        ...
        if (Math.abs(this.previousContainerWidth - totalWidth) > 1) {
          ...
          for (const p in layoutObject) {
            if (layoutObject[p] && isUnitPixelBased(layoutUnitTypes[p])) {
              const newWidth = getNewPanewidthObject(
                p,
                layoutObject[p],
                proportion,
              );
  ...
  calcWidthsAndResize = (changeType) => {
    this.widthsRAF = requestAnimationFrame(() => {
      ...
      if (toApply !== null && sizesCached === -1) {
        ...
        widths =
          this.resizeCachedSizesToFit(sizesCached) ||
          this.state.layoutCache[sizesCached];
      } else {
  ...
  calcWidths = (panes) => {
    ...
    const staticPercent =
      ((containerWidth - staticSpace) / containerWidth) * 100;
    const basePercentage =
      staticPercent / Math.max(Object.keys(dynamics).length, 1);
  ...
  cachedLayoutIndex = (candidate) => {
    return (
      this.state.layoutCache?.findIndex((cache) => {
  ...
  render() {
    const { id, children, paneset, isRoot } = this.props;
