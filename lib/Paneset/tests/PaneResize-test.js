...
describe('Pane Resizing', () => {
  ...
  describe.only('setting initial sizes', () => {
    beforeEach(async () => {
      const layouts = [{
        searchPane: '20%',
        resultPane: '20%',
      ...
      await mountWithContext(
        <Paneset id="nestedSet" initialLayouts={layouts}>
          <Pane id="searchPane"><span>Content</span></Pane>
          <Pane id="resultPane"><span>Content</span></Pane>
          <Pane id="detailPane"><span>Content</span></Pane>
          <Paneset id="nestedSet">
  ...
  describe('handles not present when overlapped', () => {
    const modalToggle = new Interactor('#toggle-modal');
    const modalClose = new Interactor('#close-modal');
    ...
    it('does not render resize handles when panes are overlapped', () => {
      expect(resize.handles(0).isPresent).to.be.false;
    });
    ...
    describe('When the overlapping element is closed', () => {
      ...
      it('resize handles return', () => {
        expect(resize.handles(0).isPresent).to.be.true;
      });
    });
  });
});
