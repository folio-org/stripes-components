...
class Paneset extends React.Component {
  ...
  maximizePane(id) {
    // find index of intended maximee in state...
    const maximeeIndex = this.state.panes.findIndex(p => p.id === id);
    const maxedWidthArray = new Array(this.state.panes.length);
    _.fill(maxedWidthArray, 0);
    maxedWidthArray[maximeeIndex] = '100%';

    // store original widths for restoration later...
    this.widthsCache = this.widths;
    this.widths = maxedWidthArray;
    // handle if nested...
    if (this.context.paneset) {
      this.context.paneset.maximizePane(this.id);
    }
    this.resizePanes(this.state.panes, maxedWidthArray);
  }

  unmaximizePane() {
    this.widths = this.widthsCache;
    // handle if nested...
    if (this.context.paneset) {
      this.context.paneset.unmaximizePane();
    }
