...
class Paneset extends React.Component {
  ...
  constructor(props) {
    ...
    this.dbCalcWidths = debounce(this.calcWidthsAndResize, 80);
  ...
  resizeCachedSizesToFit = (layoutIndex) => {
    ...
    if (!paneset) {
      ...
      if (Math.abs(this.previousContainerWidth - window.innerWidth) < 3) {
        if (Math.abs(this.previousContainerWidth - totalWidth) > 3) {
          let newCacheObject = {};
