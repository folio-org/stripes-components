  ...
  resizeToContainer = () => {
    ...
    if (matchingCacheIndex !== -1) {
      ...
      if (currentWidth) {
        ...
        const totalWidth = this.getChildPanesTotalWidth();
        ...
        this.setState({ changeType: 'windowResizeNestedPanesetPanes' },
          () => {
            this.resizeNestedTO = setTimeout(() => {
              this.state.panes.forEach((p) => {
                if (p.isPaneset) {
      ...
      // even if the width of the outside containers fills the window (currentWidth === totalWidth) the panes within
      // nested panesets may need resizing...
      this.setState({ changeType: 'windowResizeNestedUncached' },
        () => {
          this.resizeNestedTO = setTimeout(() => {
            this.state.panes.forEach((p) => {
              if (p.isPaneset) {
