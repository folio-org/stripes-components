...
class Paneset extends React.Component {
  ...
  componentDidMount() {
    ...
    this.resizeHandler = window.addEventListener('resize', debounce(this.handleWindowResize, 150));
      ...
      if (currentWidth) {
        ...
        for (const p in matchingLCache) {
          if (Object.prototype.hasOwnProperty.call(matchingLCache, p)) {
            const unit = parseCSSUnit(matchingLCache[p]);
            let newWidth = matchingLCache[p];
            switch (unit) {
              case 'percent':
              case 'vw': break;
              case 'px':
                newWidth = Math.ceil(parseInt(matchingLCache[p], 10) * proportionalChange);
                newWidth = `${newWidth}px`;
                break;
              case 'em':
                newWidth = Math.ceil(parseInt(matchingLCache[p], 10) * 14 * proportionalChange);
  ...
  setStyle = (style) => {
    if (this.isThisMounted()) {
      ...
      () => {
        setTimeout(() => { this.previousContainerWidth = this.container.current.getBoundingClientRect().width; });
      });
    }
  }
