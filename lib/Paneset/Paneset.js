...
import parseUnit from '../../util/parseCSSUnit';
    ...
    panes.forEach((pane, i) => {
      if (pane.staticWidth) {
        ...
        if (Number.isNaN(currentPaneWidth)) {
          ...
          // parse unit
          const unit = parseUnit(pane.ref.props.defaultWidth);
          // convert to pixels
          let parsedWidth;
          switch (unit) {
            case 'percent':
  ...
  render() {
    return (
      <PanesetContext.Provider value={this.state.paneManager}>
        <div className={this.getClassName()} style={this.state.style} ref={this.container}>
          {this.props.children}
        </div>
