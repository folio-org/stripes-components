...
type PaneResizeContainerProps = {
  children?: React.ReactNode[] | React.ReactNode;
  isRoot?: boolean;
  onElementResize?: (...args: any[]) => any;
  parentElement?: Element;
  resizeContainerRef?: Record<string, any> | ((...args: any[]) => any);
};

const PaneResizeContainer = ({
  isRoot,
  children,
  onElementResize,
  parentElement,
  resizeContainerRef,
}: PaneResizeContainerProps) => {
  ...
  const observerCallback = useRef((mutationList) => {
      ...
      mutationList.forEach((m) => {
        if (m.type === "childList") {
          ...
          if (typeof curBlocker === "object") {
            if (
              Array.from(m.removedNodes).find((n) => {
  ...
  useEffect(() => {
    if (isRoot) {
      if (!dragging && activeHandle !== null) {
        ...
        const resizedIndex = handleList.findIndex(
          (h) => h.id === activeHandle.id,
        );
        if (resizedIndex !== -1) {
  ...
  const renderHandles = useCallback(() => {
    ...
    const handleElements = handles.map((h) => {
      ...
      if (rect) {
        ...
        const newLeft =
          rect.left - (containerRect.current ? containerRect.current.left : 0);
        const top =
          rect.top - (containerRect.current ? containerRect.current.top : 0);
        handleData.current.push({
          id: h.id,
          elementId: h.elementId,
          x: newLeft,
