...
const PaneResizeContainer = ({ isRoot, children, onElementResize }) => {
  ...
  const registerHandle = ({ id, getRef }) => {
  ...
  const renderHandles = () => {
    ...
    const handleElements = handles.map(h => {
      ...
      if (rect.left === 0) {
        handleData.current.push({ id: h.id, x: 0 });
        return false;
      }
      ...
      handleData.current.push({ id: h.id, x: newLeft });
  ...
  if (!isRoot) {
    return (
      <>
        <ResizeProvider value={{ registerHandle, updateHandle }}>
          {children}
        </ResizeProvider>
        <div
          ref={container}
