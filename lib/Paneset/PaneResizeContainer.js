...
const PaneResizeContainer = ({ isRoot, children, onElementResize, parentElement }) => {
  ...
  const observerCallback = useRef(
    (mutationList) => {
      // console.log(`mutations happening: ${mutationList.length}`);
      setBlocking(curBlocker => {
        ...
        mutationList.forEach((m) => {
          if (m.type === 'childList') {
            if (m.addedNodes.length > 0) {
              ...
              m.addedNodes.forEach((n) => {
                ...
                if (
                  ...
                  /* store multiple attributes from the blocker... although
                    they probably have an id, there's a chance it's just left at ""
                    which doesn't give much to prove on when trying to return
                    the resize functionality. Classname or style - one of these will
                    need to be filled in order for the element to cover the paneset.
                  */
                  blocker = {
                    id: n.id,
                    class: n.className,
            ...
            if (m.removedNodes.length > 0 && typeof blocker === 'object') {
