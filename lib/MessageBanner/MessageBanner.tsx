...
const MessageBanner = forwardRef(
  (
    ...
    const renderIcon = () => {
      let iconName =
        typeof type === "string" && DEFAULT_ICONS[type.toUpperCase()];
    ...
    const getMessageBannerClass = () =>
      classnames(css.root, { [css.marginTop0]: marginTop0 });

    return (
      <div
        role="alert"
        aria-live={ariaLive}
        className={getMessageBannerClass()}
      >
        <CSSTransition
          in={visible}
          timeout={200}
        ...
        >
          <Element
            ...
            className={classnames(css.inner, css[`type-${type}`], className)}
            ref={ref}
            {...rest}
          >
            {renderIcon()}
                  ...
                  {([dismissAriaLabel]) => (
                    <IconButton
                      ...
                      className={classnames(
                        css.dismissButton,
                        get(dismissButtonProps, "className"),
                      )}
                      size="medium"
                      onClick={onDismiss}
                      icon="times"
