...
const cumulativeCellWidth = async (mcl) => {
  ...
  const widths = await Promise.all([...Array(nColumns)].map(async (_, i) => {
    let width = 0;
    await mcl.find(CellInteractor({ row: 0, column: i })).perform(el => { width = el.offsetWidth; });
    return width;
  }));
...
describe.only('MultiColumnList', () => {
  ...
  describe('column width ranges', () => {
    ...
    it('keeps widths between the specified range', async () => {
      ...
      await mcl.find(CellInteractor({ row: 0, column: 0 })).perform(assertWidthWithin(24, 24));
      await mcl.find(CellInteractor({ row: 0, column:1 })).perform(assertWidthWithin(0, 60));
      await mcl.find(CellInteractor({ row: 0, column: 2 })).perform(assertWidthWithin(60, 80));
      await mcl.find(CellInteractor({ row: 0, column: 3 })).perform(assertWidthWithin(60, 80));
    });
  });
  ...
  describe('Basic data rendering', () => {
    ...
    it('renders the proper content in each row', () => Promise.all(
      xyRange(data).map(async (xy) => {
        await mcl.find(CellInteractor({ row: xy[0], column: xy[1] })).perform(el => {
  ...
  describe('rendering with columnWidths prop', () => {
    ...
    it('renders columns with requested Width', async () => {
      await mcl.find(CellInteractor({ row: 0, column: 0 })).perform(el => {
        expect(el.offsetWidth).to.equal(100);
      });
      await mcl.find(CellInteractor({ row: 0, column: 1 })).perform(el => {
        expect(el.offsetWidth).to.equal(150);
      });
      await mcl.find(CellInteractor({ row: 0, column: 2 })).perform(el => {
  ...
  describe('data change from sorting', () => {
    ...
    it('renders data in appropriate cell', async () => {
      await mcl.find(CellInteractor({ row: 0, column: 0 })).has({ content: data5[0].name });
    });
    ...
    describe('clicking a header', () => {
      ...
      it('shifts the data down one row', async () => {
        await mcl.find(CellInteractor({ row: 1, column: 0 })).has({ content: data5[0].name });
      });
      ...
      describe('click a header again', () => {
        ...
        it('shifts the data down another row', async () => {
          await mcl.find(CellInteractor({ row: 2, column: 0 })).has({ content: data5[0].name });
        });
      });
    });
  });
  ...
  describe('rendering with virtualization', () => {
    ...
    describe('scrolling the MCL', () => {
      ...
      it('advances the rendered slice of rows', async () => {
        await mcl.find(CellInteractor({ row: 0, column: 0 })).assert((el) => el.textContent !== data100[0].id);

        // we are not rendering the full data set so the last shouldn't appear
        await mcl.find(CellInteractor({ row: data100.length - 1, column: 0 })).absent();
      });
    });
  });
  ...
  describe('rendering with a hidden grid', () => {
    ...
    it('cells are not measured', () => mcl.find(CellInteractor({ row: 2, column: 0 })).is({ measured: false }));
    ...
    describe('revealing the grid', () => {
      ...
      it('cells are measured', () => mcl.find(CellInteractor({ row: 2, column: 0 })).is({ measured: true }));
    });
  });
  ...
  describe('rendering with empty starting data', () => {
    ...
    describe('receiving data', () => {
      ...
      // TODO it is either not clicking or not setting the selected class
      // unsure what the issue is here
      describe.skip('selecting a row', () => {
        beforeEach(async () => {
          await mcl.find(CellInteractor({ row: 2, column: 1, measured: true })).click();
        });

        it('applies the selected class', () => mcl.find(CellInteractor({ row: 2, column: 1 })).is({ selected: true }));
        ...
        describe('adjusting to width change', () => {
          ...
          describe('selecting another', () => {
            beforeEach(async () => {
              await mcl.find(CellInteractor({ row: 11, column: 1 })).click();
            });

            it('applies the class to the updated selection', () => mcl.find(CellInteractor({ row: 11, column: 1 })).is({ selected: true }));
          });
        });
      });
    });
  ...
  describe('using formatter prop', () => {
    ...
    describe('receiving data', () => {
      ...
      it('renders the formatter', () => mcl.find(CellInteractor({ row: 3, column: 0 })).assert((el) => !!el.querySelector('[data-test-date-joined]')));
    });
  });
  ...
  describe('passing rowProps with "to" as string', () => {
    ...
    it('renders default formatter (rows as html anchors, <a>)', () => mcl.find(CellInteractor({ row: 0, column: 0 }))
      .assert(
