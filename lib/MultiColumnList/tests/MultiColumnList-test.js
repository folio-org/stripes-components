...
describe('MultiColumnList', () => {
  ...
  describe('column width ranges', () => {
    ...
    it('keeps widths between the specified range', () => {
      expect(mcl.rows(0).cells(0).width).to.be.within(24, 24);
      expect(mcl.rows(0).cells(1).width).to.be.within(0, 60);
      expect(mcl.rows(0).cells(2).width).to.be.within(60, 80);
      expect(mcl.rows(0).cells(3).width).to.be.within(60, 80);
    });
  });
    ...
    it('renders the proper content in each row', () => {
      ...
      data.forEach((d, n) => {
        ...
        for (let j = 0; j < mcl.rows(n).cellCount; j++) {
          if (mcl.rows(n).cells(j).content.toString() !== d[Object.keys(d)[j]].toString()) {
            correctData = false;
            // console.log(`expected: ${mcl.rows(n).cells(j).content}, actual: ${d[Object.keys(d)[j]]}`);
  ...
  describe('rendering with columnWidths prop', () => {
    ...
    it('renders columns with requested Width', () => {
      ...
      colKeys.forEach((n, i) => {
        ...
        } else { // last column shouldbe flex 1 1 auto to fill remaining space.
          matches = (mcl.rows(0).cells(i).$root.style.flex === '1 1 auto');
        }
      });
  ...
  describe('rendering with partial columnWidths and no width prop', () => {
    ...
    it('container width 400px: row width == sum of column widths', () => {
      const combinedCellWidths = mcl.rows(0).cells().reduce((n, i) => n + i.width, 0);
      expect(`${combinedCellWidths}px`).to.equal(mcl.rows(0).minWidth);
    });
    ...
    describe('rendering with partial columnWidths and no width prop', () => {
      ...
      it('container width 1000px: row width == container width', () => {
        expect('1000px').to.equal(mcl.rows(0).minWidth);
      });
    });
  });
  ...
  describe('Percentage widths that sum to 100% ', () => {
    ...
    it('does not cause overflow', () => {
      const combinedCellWidths = mcl.rows(0).cells().reduce((n, i) => n + i.width, 0);
      expect(combinedCellWidths).to.be.at.most(400);
    });
  });
  ...
  describe('using the scrollToIndex prop', () => {
    let pagingNeedMore;
    beforeEach(async () => {
      pagingNeedMore = sinon.fake();
      await mountWithContext(
        <div style={{ height: '300px', width: '600px' }}>
          <MultiColumnList contentData={data100} scrollToIndex={50} onNeedMoreData={pagingNeedMore} autosize />
        </div>
      );
    });

    it('renders the row container with a large enough height', () => {
      expect(mcl.scrollBodyHeight).to.be.gt(1500);
    });

    it('scrolls the scrollable container to the coordinating pixel coordinate', () => {
      expect(mcl.scrollTop).to.be.gt(1500);
    });
  });
