...
describe('MultiColumnList', () => {
  ...
  describe('prev-next paging when the pagination row is taller than the default reserved height', () => {
    let instance;
    let needMoreHandler;
    ...
    it('measures the true pagination row height on mount instead of relying on the 40px default', () => {
      expect(instance.state.paginationHeight).to.equal(instance.paginationContainer.current.offsetHeight);
    });

    describe('measureHeaderAndPaginationHeights', () => {
      it('reports only the keys whose measured value actually changed', () => {
        const stub = sinon.stub(instance.paginationContainer.current, 'offsetHeight').get(() => 123);
        expect(instance.measureHeaderAndPaginationHeights()).to.eql({ paginationHeight: 123 });
        stub.restore();
      });

      it('returns an empty diff once state matches the DOM, so re-measuring cannot loop forever', () => {
        const stub = sinon.stub(instance.paginationContainer.current, 'offsetHeight').get(() => 123);
        instance.setState(instance.measureHeaderAndPaginationHeights());
        expect(instance.measureHeaderAndPaginationHeights()).to.eql({});
        stub.restore();
      });
    });

    describe('when the pagination row grows taller after mount (e.g. a wrapped button label)', () => {
      let scrollableHeightBefore;
      let stub;

      beforeEach(async () => {
        scrollableHeightBefore = instance.scrollContainer.current.offsetHeight;
        stub = sinon.stub(instance.paginationContainer.current, 'offsetHeight').get(() => 90);

        // clicking Next runs handleLoadMore -> setState({ loading: true }), which is enough
        // to drive componentDidUpdate and re-measure header/pagination heights.
        await mcl.clickNextPagingButton();
      });
      ...
      it('updates state.paginationHeight to the newly measured value', () => {
        expect(instance.state.paginationHeight).to.equal(90);
      });

      it('shrinks the reserved scrollable rows area so the taller pagination row is not clipped', () => {
        expect(instance.scrollContainer.current.offsetHeight).to.be.below(scrollableHeightBefore);
      });
    });
  });
