...
describe.only('MultiColumnList', () => {
  ...
  describe.only('rendering with partial columnWidths and no width prop', () => {
    const testWidths = {
      name: '25%',
      email: '150px',
    };
    ...
    it('row width == sum of column widths', () => {
      const combinedCellWidths = mcl.rows(0).cells().reduce((n, i) => n + i.width, 0);
      console.log(combinedCellWidths);
      console.log(mcl.rows(0).minWidth);
      expect(combinedCellWidths).to.equal(mcl.rows(0).minWidth);
    });
  });
