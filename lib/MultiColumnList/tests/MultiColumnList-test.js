...
describe.only('MultiColumnList', () => {
  ...
  describe('rendering with virtualization', () => {
    ...
    describe('scrolling the MCL', () => {
      ...
      it('advances the rendered slice of rows', async () => {
        await mcl.find(CellInteractor({ xy: [0, 0] })).assert((el) => el.textContent !== data100[0].id);

        // we are not rendering the full data set so the last shouldn't appear
        await mcl.find(CellInteractor({ xy: [data100.length - 1, 0] })).absent();
      });
    });
  });
  ...
  describe('rendering with a hidden grid', () => {
    ...
    it('cells are not measured', () => mcl.find(CellInteractor({ xy: [2, 0] })).is({ measured: false }));
    ...
    describe('revealing the grid', () => {
      ...
      it('cells are measured', () => mcl.find(CellInteractor({ xy: [2, 0] })).is({ measured: true }));
    });
  });
  ...
  describe('rendering with empty starting data', () => {
    ...
    describe('receiving data', () => {
      ...
      describe.skip('selecting a row', () => {
        beforeEach(async () => {
          await mcl.find(CellInteractor({ xy: [2, 1], measured: true })).click();
        });

        it('applies the selected class', () => mcl.find(CellInteractor({ xy: [2, 1] })).is({ selected: true }));
        ...
        describe('adjusting to width change', () => {
          ...
          describe('selecting another', () => {
            beforeEach(async () => {
              await mcl.find(CellInteractor({ xy: [11, 1] })).click();
            });

            it('applies the class to the updated selection', () => mcl.find(CellInteractor({ xy: [11, 1] })).is({ selected: true }));
          });
        });
      });
    });
