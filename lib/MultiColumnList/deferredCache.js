export default class Cache {
  ...
  requestAccumulated = (index, cb) => {
    ...
    if (index < n) {
      ...
      if (n === index) {
        ...
        return cb ? cb({ result:currentLowAccum, estimate: false }) : currentLowAccum;
      }

      if (cb) {
        this.accumRequests[index] = cb;
        // const res = index * this.fillerItem + ((this.highestAccumIndex * this.fillerItem) - this.highestAccum);
        const res = this.lowestAccum - ((this.lowestAccumIndex - index) * this.fillerItem);
  ...
  forEach = (func) => {
    return Object.keys(this.cache).forEach(k => func(this.cache[k]));
  };
  ...
  rebase = (index, amount) => {
    ...
    const remaining = this.cache[index];
    this.cache = { [index] : remaining };
    this.length = 1;
