...
class MultiColumnList extends React.Component{
  ...
  maybeSelectedClass(criteria, i){
    const testItem = this.props.contentData[i];
    let selected = Object.keys(criteria).length > 0;
  ...
  render(){
    ...
    contentData.map(function(item, i){
      ...
      //assemble rows...
      listItems.push(<tr key={i} data-row={i} className={this.maybeSelectedClass(selectedRow, i)} onClick={this.handleRowClick}>{listItem}</tr>);
