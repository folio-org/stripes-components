...
type MCLRendererProps = {
  autosize?: boolean;
  columnIdPrefix?: string;
  columnMapping?: Record<string, any>;
  columnOverflow?: Record<string, any>;
  columnWidths?: Record<string, any>;
  containerRef?: ((...args: any[]) => any) | Record<string, any>;
  contentData?: Record<string, any>[];
  dataEndReached?: boolean;
  dndProvided?: { placeholder?: React.ReactNode };
  formatter?: Record<string, any>;
  getCellClass?: (...args: any[]) => any;
  getHeaderCellClass?: (...args: any[]) => any;
  getRowContainerClass?: (...args: any[]) => any;
  hasMargin?: boolean;
  ...
  id?: string;
  instanceRef?: (...args: any[]) => any;
  interactive?: boolean;
  intl?: { formatMessage?: (...args: any[]) => any };
  isEmptyMessage?: string | Record<string, any> | React.ReactNode | any[];
  isSelected?: (...args: any[]) => any;
  itemToView?: { localClientTop?: number; selector?: string };
  loading?: boolean;
  maxHeight?: number;
  minimumRowHeight?: number;
  nonInteractiveHeaders?: string[];
  onHeaderClick?: (...args: any[]) => any;
  onMarkPosition?: (...args: any[]) => any;
  onMarkReset?: (...args: any[]) => any;
  onNeedMoreData?: (...args: any[]) => any;
  onRowClick?: (...args: any[]) => any;
  onScroll?: (...args: any[]) => any;
  pageAmount?: number;
  pagingButtonLabel?: React.ReactNode;
  pagingCanGoNext?: boolean;
  pagingCanGoNextLoading?: boolean;
  pagingCanGoPrevious?: boolean;
  pagingCanGoPreviousLoading?: boolean;
  pagingOffset?: number;
  pagingType?: any;
  rowFormatter?: (...args: any[]) => any;
  rowMetadata?: string[];
  rowProps?: Record<string, any>;
  rowUpdater?: (...args: any[]) => any;
...
export function dataChangedOrLess(previous, current) {
  ...
  for (
    let i = Number.parseInt(previousKeys[0], 10);
    i < Number.parseInt(previousKeys[previousKeys.length - 1], 10);
    i += interval
  ) {
    const notEqual = !isEqualWith(
      previous[i] || {},
      current[i] || {},
      comparison,
    );
  ...
  if (!isEqualWith(current[prevLast], previous[prevLast], comparison)) {
    return {
      changed: true,
      changeType:
        previous[prevLast] === null ? CHANGE_NEWDATA : CHANGE_DIFFERENCE,
      datalength: currentKeys.length,
    };
  }
...
function computeRowWidth(columnWidths, columns) {
  return Object.keys(columnWidths).reduce((sum, k) => {
    const rowWidth = columns.includes(k)
      ? sum + parseFloat(columnWidths[k], 10)
      : sum;
    return rowWidth;
  }, 0);
}
...
class MCLRenderer extends React.Component<MCLRendererProps> {
  static defaultProps = {
  ...
  constructor(props) {
    ...
    this.debouncedLoadMore = debounce(this._loadMore.bind(this), 200, {
      trailing: true,
      leading: true,
    });
    ...
    this.handlers = Object.assign(
      {
        selectPreviousRow: (e) => {
          baseHandlers.selectPreviousRow(e);
        },
  ...
  static getDerivedStateFromProps(props, state) {
    const { columns, receivedRows, prevHeight, prevWidth } = state;
    ...
    if (
      ...
      if (visibleColumns) {
        ...
        newState.columns = initColumnsFromData(
          contentData[0] || {},
          headerMetadata,
          rowMetadata,
        );
    ...
    if (columnWidths && width && width !== state.prevWidth) {
      ...
      if (
        ...
        newState.rowWidth = computeRowWidth(
          newState.columnWidths,
    ...
    if (contentData && contentData.length !== 0) {
      ...
      newState.dataEndIndex = Number.parseInt(
    ...
    if (newState.isSparse && receivedRows !== checkValue) {
      ...
      if (!virtualize) {
        ...
        newState.lastIndex =
          Number.parseInt(
            contentDataIndices[contentDataIndices.length - 1],
            10,
          ) || 0;
  ...
  componentDidMount() {
    const { contentData, width } = this.props;
    ...
    if (contentData.length > 0) {
      ...
      newState.hasScrollbar = this.scrollContainer.current
      ...
      if (Object.keys(newState).length > 0) {
        this.setState((curState) => {
          if (
            newState.columnWidths &&
            Object.keys(newState.columnWidths).length !==
              Object.keys(curState.columnWidths).length
          ) {
            newState.columnWidths = Object.assign(
              {},
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      prevProps.contentData.length &&
      this.props.contentData.length > prevProps.contentData.length
    ) {
    ...
    if (shouldUpdate.changed && shouldUpdate.changeType !== CHANGE_NEWDATA) {
      ...
      if (
        shouldUpdate.changeType === CHANGE_DIFFERENCE ||
        shouldUpdate.changeType === CHANGE_LESSDATA
      ) {
      ...
      if (!isSparse) {
        ...
        newState.firstIndex =
          Number.parseInt(Object.keys(contentData)[0], 10) || 0;
        newState.loading = false;
      }
    } else if (
      pagingType === pagingTypes.LOAD_MORE ||
      ...
      if (!isNil(this.focusTargetIndex) && !loading) {
        ...
        const target = current?.querySelector(
          `[data-row-index="row-${targetIndex}"]`,
        );
    ...
    const possibleRowWidth = this.memoizedComputeRowWidth(
      this.state.columnWidths,
      columns,
    );
    ...
    if (Object.keys(newState).length > 0) {
      ...
      this.setState((curState) => {
        ...
        if (
          newState.columnWidths &&
          Object.keys(newState.columnWidths).length !== curState.columns.length
        ) {
          newState.columnWidths = Object.assign(
            {},
            newState.columnWidths,
  ...
  scrollToItemToView = () => {
    ...
    const { itemToView, onMarkReset, virtualize } = this.props;
  ...
  handleScroll = (e) => {
    ...
    this.scrollRAF = requestAnimationFrame(() => {
      if (this.headerRow.current && this.scrollContainer.current) {
        this.headerContainer.current.scrollLeft =
  ...
  handleInfiniteScroll = (currentScroll, currentScrollLeft, index) => {
    ...
    if (!index) {
      let startPosition =
        currentScroll -
        (minimumRowHeight + this.maximumRowHeight + currentScrollDelta);
      if (startPosition < 0) startPosition = 0;
      nextState.firstIndex = this.rowHeightCache.getIndexByPosition(
        startPosition,
        minimumRowHeight,
      );
  ...
  getRowClass = (rowIndex) => {
    ...
    return classnames(
      ...
      // Is selected
      {
        [`${selectedClass}`]: this.maybeSelected(
  ...
  renderDataRow = (rowIndex) => {
    ...
    const { staticBody, columns, prevWidth, columnWidths, rowWidth } =
      this.state;
    ...
    const onClick = onRowClick
      ...
      : undefined;
    const onKeyDown = (e) =>
      this.handleRowKeyPress(e, this.getRowData(rowIndex));
  ...
  renderLoaderRow = (rowIndex, dataRowsRendered, heightIncrement) => {
    const { contentData, totalCount, pageAmount } = this.props;
  ...
  renderEndOfList = (endIndex, renderPosition) => {
    const { dataEndReached, id, virtualize, totalCount, width } = this.props;

    const { prevWidth, staticBody } = this.state;
        ...
        {({ position }) => {
          return (
            <div
              ...
              className={css.mclEndOfListContainer}
            ...
            >
              <CenteredContainer
                ...
                visible={
                  virtualize &&
                  (dataEndReached ||
                    (totalCount > 0 && totalCount <= renderPosition))
                }
              >
                <EndOfList />
              </CenteredContainer>
  ...
  renderPagingRow = (rowIndex) => {
    ...
    const { prevWidth, loading, staticBody } = this.state;
  ...
  renderRows = () => {
    ...
    */
    ...
    for (
      ...
      (virtualize && totalCount ? rowIndex < totalCount : true);
      ...
      if (contentData[rowIndex] && rowIndex < contentData.length) {
        ...
        loaderSettings.load &&
        (totalCount === 0 || rowIndex < totalCount)
      ) {
        if (totalCount === 0) {
          currentTop = bodyExtent;
        }
  ...
  renderCells = (rowIndex, rowData) => {
    ...
    columns.forEach((col, colIndex) => {
      ...
      const cellStyleClass =
        typeof getCellClass === "function"
          ? getCellClass(css.mclCellStyle, rowData, col)
          : css.mclCellStyle;
      ...
      if (columnWidths[col]) {
        ...
        if (stickyLastColumn && colIndex === columns.length - 1)
      ...
      // eslint-disable-next-line
      const shouldMeasure =
        (modColumns.includes(col) && !columnWidths[col]) || !colWidthsVar;
  ...
  maybeUpdateColumnWidths = (rowIndex, columnName, width) => {
    ...
    if (
      this.widthCache[columnName].length >= sample &&
      ...
      if (typeof cwProp[columnName] === "object")
        columnModifiers = cwProp[columnName];
  ...
  getHeaderClassName = (column, sortedBy, isInteractive) => {
    const { showSortIndicator, sortableFields } = this.props;
    ...
    return classnames(
      ...
      {
        [`${css.mclAscending}`]:
          sortedBy && this.props.sortDirection === "ascending",
      },
      {
        [`${css.mclDescending}`]:
          sortedBy && this.props.sortDirection === "descending",
      },
      {
        [`${css.mclSortIndicator}`]:
          showSortIndicator && isSortable && !sortedBy,
      },
    );
  };
  ...
  renderHeaders = () => {
    ...
    columns.forEach((header, i) => {
      ...
      const isInteractive =
        onHeaderClick && !nonInteractiveHeaders.includes(header);

      const headerWidth = columnWidths[header];

      const defaultHeaderCellClass = this.getHeaderClassName(
        header,
        isSortHeader,
        isInteractive,
      );
      ...
      if (headerWidth) {
        ...
        const customHeaderClass =
          typeof getHeaderCellClass === "function" &&
          getHeaderCellClass(header);
        headerCellClass = customHeaderClass
          ? `${defaultHeaderCellClass} ${customHeaderClass}`
        ...
        headerInner = (
          ...
          >
            <div className={css.mclHeaderInner}>
              <div
                style={{ flex: "0 100 content", maxWidth: "calc(100%-15px)" }}
              >
      ...
      const needsMeasuring =
        modColumns.includes(header) && headerMeasured === null;
      const noWidthHintProvided =
        !columnWidthsProp[header] && this.headerWidths.request(header) === null;
  ...
  handleLoadMore = (askAmount, index, direction) => {
    if (
      index === 0 ||
      index === this.state.firstindex ||
      index % this.props.pageAmount !== 0
    ) {
  ...
  handleMoreResultsLoaded = () => {
    ...
    this.sendMessage(
      formatMessage({ id: "stripes-components.mcl.additionalResultsLoaded" }),
    );
  };
  ...
  getRowContainerStyle = () => {
    ...
    if (height !== undefined) {
      ...
      const paginationHeight =
        pagingType === pagingTypes.PREV_NEXT ? this.paginationHeight : 0;

      newHeight = Math.max(
        (pagingType !== pagingTypes.SCROLL
          ? receivedRows
  ...
  getScrollableStyle = () => {
    const { height, maxHeight, pagingType } = this.props;
    ...
    if (bodyHeight && maxHeight) {
      scrollableStyle.height = Math.min(
        bodyHeight,
        maxHeight - this.headerHeight - paginationHeight,
      );
    } else if (heightVar > 0) {
  ...
  render() {
      ...
      return (
        ...
        >
          {typeof isEmptyMessage === "string" ? (
            <EmptyMessage>{isEmptyMessage}</EmptyMessage>
          ) : (
            isEmptyMessage
          )}
        </div>
    ...
    const rowContainerClass =
      typeof getRowContainerClass === "function"
          ...
          >
            ...
            >
              <div
                ...
                className={css.mclHeaderContainer}
              >
                <div
            ...
            {pagingType === pagingTypes.PREV_NEXT && (
              <PrevNextPaginationRow
                ...
                virtualize={virtualize}
                dataStartIndex={
                  typeof pagingOffset !== "undefined"
                    ? pagingOffset + 1
                    : dataStartIndex + 1
                }
                dataEndIndex={
                  typeof pagingOffset !== "undefined"
                    ? pagingOffset + dataEndIndex + 1
                    : dataEndIndex + 1
                } // eslint-disable-line
