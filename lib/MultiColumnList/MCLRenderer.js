...
export function dataChangedOrLess(previous, current) {
  ...
  if (current.length < previous.length) return { changed: true, changeType: CHANGE_LESSDATA, populatedLength: current.length };
...
class MCLRenderer extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (contentData !== undefined) {
      if (contentData.hasOwnProperty[0] && contentData.hasOwnProperty[contentData.length - 1]) {
    ...
    const checkValue = newState.isSparse ? loadedEstimate : contentData.length;

    if (receivedRows !== checkValue) {
      newState.receivedRows = checkValue;
      newState.loading = false;

      if (checkValue < receivedRows) {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (shouldUpdate.changed && shouldUpdate.changeType !== CHANGE_NEWDATA) {
      ...
      shouldUpdate.datalength > prevState.receivedRows
    ) {
      newState.loadedEstimate = shouldUpdate.datalength;
    }
