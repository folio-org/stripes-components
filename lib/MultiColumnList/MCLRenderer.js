...
class MCLRenderer extends React.Component {
  ...
  getHeaderClassName(column, sortedBy) {
    return classnames(
      ...
      { [`${css.sorted}`]: sortedBy },
      { [`${css.ascending}`]: (sortedBy && this.props.sortDirection === 'ascending') },
      { [`${css.descending}`]: (sortedBy && this.props.sortDirection === 'descending') },
    );
  }
  ...
  generateHeaders() {
    const { columnWidths } = this.state;
    const { onHeaderClick, sortDirection, sortedColumn } = this.props;
    const headers = [];
    this.state.columns.forEach((header, i) => {
      ...
      const isSortHeader = (this.props.sortOrder === this.getMappedColumnName(header) ||
      this.props.sortedColumn === this.getMappedColumnName(header));
    ...
    return (
      <HotKeys handlers={this.handlers} noWrapper>
        <div
          ...
          ref={this.serveContainerRef}
          role="grid"
          aria-rowCount={totalCount || contentData.length}
