...
class MCLRenderer extends React.Component {
  ...
  updateDimensions(height, data, averageRowHeight) {
    let avgHeight = averageRowHeight;
    if (!avgHeight) {
  ...
  handleHeaderClick(e, name) {
    ...
    if (this.props.headerMetadata) {
      for (const prop in this.props.headerMetadata[name]) {
  ...
  measureColumns({ contentData, cols, columnWidths }) {
    let data = contentData;
    let columns = cols;
    ...
    columns.forEach((colName) => {
      if (typeof colWidths !== 'undefined' && Object.prototype.hasOwnProperty.call(colWidths, colName)) {
