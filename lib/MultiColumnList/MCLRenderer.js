...
class MCLRenderer extends React.Component {
  ...
  renderDataRow = (rowIndex) => {
    ...
    /* removes unnecessary horizontal scroll
    *  due to CSS margins on .mclRow
    */
    const rowMargin = 5;
    /* set minWidth of rows to either the width (minus margin)
        ...
        {({ localRowIndex, position }) => (
          ...
          >
            ...
            >
              <Button
                disabled={loading}
                onClick={() => this.handleLoadMore(pageAmount, rowIndex)}
                style={{ width: '75%' }}
              >
