...
class MCLRenderer extends React.Component {
  ...
  constructor(props) {
    ...
    this.positionCache = new DimensionCache(30, 0, 0);
    this.rowHeightCache = new DimensionCache(30, 0, 0);
  ...
  handleInfiniteScroll = (currentScroll, currentScrollLeft, index) => {
    ...
    if (!index) {
      let startPosition = currentScroll - (this.props.minimumRowHeight + this.maximumRowHeight + currentScrollDelta);
      if (startPosition < 0) startPosition = 0;
      const average = this.state.averageRowHeight || this.updateAverageHeight();
      nextState.firstIndex = this.rowHeightCache.getIndexByPosition(startPosition, average);
    ...
    for (
      ...
      if (contentData[rowIndex]) {
        ...
        if (totalCount === 0) {
          ...
          if (contentData.length - (firstIndex + rowsRendered) < 30) {
            rows.push(
              <LoaderRow
