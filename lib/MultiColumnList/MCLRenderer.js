  ...
  return Object.keys(columnWidths)
    .reduce((sum, k) => {
      const rowWidth = columns.includes(k) ? sum + parseFloat(columnWidths[k], 10) :
        sum;
      return rowWidth;
      // return sum + parseFloat(columnWidths[k], 10);
    }, 0);
}
...
class MCLRenderer extends React.Component {
  ...
  renderCells = (rowIndex) => {
    ...
    columns.forEach((col, colIndex) => {
      ...
      if (modColumns.includes(col)) {
        if (!columnWidths[col]) {
          shouldMeasure = true;
        }
      } else if (!colWidthsVar) {
        shouldMeasure = true;
      }
  ...
  renderHeaders = () => {
    ...
    columns.forEach((header, i) => {
      ...
      if (modColumns.includes(header) && headerMeasured === null) {
        shouldMeasure = true;
      } else if (!columnWidthsProp[header]
        && this.headerWidths.request(header) === null) {
        shouldMeasure = true;
      }
