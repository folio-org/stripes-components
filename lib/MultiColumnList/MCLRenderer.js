...
export function dataChangedOrLess(previous, current) {
  ...
  const prevPopulatedLength = previous.filter(() => true).length;
  const populatedDataLength = current.filter(() => true).length;
  if (populatedDataLength < prevPopulatedLength) {
    return { changed: true, changeType: CHANGE_LESSDATA, populatedLength: populatedDataLength };
  }
  ...
  */
  ...
  return { changed: false, changeType: null, datalength: populatedDataLength };
}
...
class MCLRenderer extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      columns: null,
      receivedRows: 0,
      firstIndex: props.virtualize ? scrollToIndex : 0,
      scrollTop: 0,
      loading: false,
      columnWidths: {},
      averageRowHeight: 0,
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (contentData !== undefined) {
      ...
      if (Object.prototype.hasOwnProperty('0', contentData) && Object.prototype.hasOwnProperty(toString(contentData.length - 1), contentData)) {
        newState.isSparse = false;
      } else {
    ...
    if (receivedRows !== checkValue) {
      ...
      if (!virtualize && newState.isSparse) {
        const contentDataIndices = Object.keys(contentData);
        newState.firstIndex = parseInt(contentDataIndices[0], 10);
        newState.lastIndex = parseInt(contentDataIndices[contentDataIndices.length - 1], 10);
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (shouldUpdate.changed && shouldUpdate.changeType !== CHANGE_NEWDATA) {
      ...
      if (!isSparse) {
        ...
        newState.firstIndex = parseInt(Object.keys(contentData)[0], 10);
      }
      ...
      >
        { /*  rowIndex passed through to children as localRowIndex since
          ...
          ({ localRowIndex, position }) => {
            ...
            if (askAmount < 0 || totalCount === contentData.length) askAmount = pageAmount > totalCount ? totalCount : pageAmount;
            return (
              <LoaderRow
  ...
  renderPagingRow = (rowIndex) => {
    ...
    if (pagingType === 'click') {
      ...
      let canGoPrevious;
      let canGoNext;
      if (this.state.isSparse) {
        canGoPrevious = !contentData[0];
        canGoNext = !contentData[contentData.length - 1];
      } else {
        // need to depend on props from outside for this.
        return null;
      }
