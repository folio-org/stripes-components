...
// used in all margin calculations... static based on CSS value of 5px. (5px * 2 = 10px)
const ROWMARGIN = 10;
...
export function dataChangedOrLess(previous, current) {
  ...
  if (current.length < previous.length) return { changed: true, changeType: 'length' };
  ...
  */
  ...
  for (let i = 0; i < previous.length - 1; i += interval) {
    ...
    if (notEqual) {
      return {
        changed: true,
        changeType: previous[i] === null ? 'new data' : 'difference'
      };
    }
  }
  ...
  if (!isEqualWith(current[prevLast], previous[prevLast], comparison)) {
    return {
      changed: true,
      changeType: previous[prevLast] === null ? 'new data' : 'difference'
    };
  }
  return { changed: false, changeType: null };
}
...
function getPixelColumnWidths(columnWidths, width) {
  ...
  const calculatedWidth = width - ROWMARGIN;
...
class MCLRenderer extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (shouldUpdate.changed) {
      ...
      if (shouldUpdate.changeType !== 'new data') {
  ...
  renderHeaders = () => {
    const { columnWidths, columns } = this.state;
    const { onHeaderClick, sortDirection, sortOrder, sortedColumn, id, columnWidths: columnWidthsProp } = this.props;
    const headers = [];
