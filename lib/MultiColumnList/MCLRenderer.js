...
class MCLRenderer extends React.Component {
  ...
  constructor(props) {
    ...
    // default itemToView to the top row of the table body (aria-rowindex: 1-based, headers are row 1).
    this.itemToView = props.itemToView || {
      selector: '[aria-rowindex="2"]',
      localClientTop: 0,
    }
    this.prevVScroll = 0;
  }
    ...
    if (item &&
      ...
      if (itemElem) {
        ...
        if (this.props.onMarkPosition) {
          if (!isEqual(this.itemToView, itemToView)) {
