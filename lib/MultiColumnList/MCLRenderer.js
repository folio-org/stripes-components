...
// Static-mode (virtualize=false) rows may carry content-visibility:auto (see .mclContentVisibility
// in MCLRenderer.css). While an off-screen row is skipped, its own measured rect can reflect the
// placeholder contain-intrinsic-size rather than its real content height, and focusable descendants
// inside it measure as 0x0 - both of which corrupt scroll-restoration math and focus-target search.
// This forces the element back to normal rendering for the duration of a synchronous measurement/
// focus-search call, then restores the previous value. checkVisibility short-circuits the common
// case where el isn't currently skipped at all (e.g. it's already on-screen, such as a row the
// user just clicked/focused) - avoids both the unnecessary style mutation and the browser's
// "forced rendering of a skipped subtree" diagnostic when there's nothing to force.
function withContentVisibilityVisible(el, fn) {
  if (!el) return fn();
...
class MCLRenderer extends React.Component {
  static propTypes = {
    ...
    // Experimental: applies content-visibility:auto to rows, but only takes effect when
    // virtualize is false - MCL's own virtualized windowing already unmounts off-screen
    // rows and self-measures the mounted ones, which content-visibility would conflict with.
    // virtualize=false is the common configuration in practice (virtualize=true breaks
    // native find-in-page and can cause flaky automated tests, so it's discouraged), so
    // this is the primary path for MCL rendering-performance work, staged as an opt-in
    // while the technique itself is validated on real, high-row-count screens.
    experimentalContentVisibility: PropTypes.bool,
  ...
  static defaultProps = {
    ...
    experimentalContentVisibility: true,
  }
  ...
  constructor(props) {
    ...
    // Authoritative source of truth for container-width changes (replaces the old
    // window-resize-only listener, which could only catch resizes caused by the window
    // itself, and unlike componentDidUpdate's old unconditional offsetWidth read, this
    // doesn't force a synchronous layout read of a subtree that may contain
    // content-visibility-skipped rows.
    this.resizeObserver = new ResizeObserver(debounce(this.handleContainerResize, 100));
    // Supplements the componentDidUpdate heuristic gate below with a precise, event-driven
    // trigger for content-visibility:auto rows (experimentalContentVisibility) - debounced
    // since a fast scroll can transition many rows' skip state in quick succession, and
    // re-measuring on every single one would reintroduce forced-layout thrashing.
    this.debouncedMeasureHeaderAndScrollbar = debounce(() => {
      const scrollbarUpdate = this.measureHeaderAndScrollbar();
      if (scrollbarUpdate) {
        this.setState(scrollbarUpdate);
      }
    }, 100);
    ...
    this.widthCache = {};
    this.shouldScrollToItemToView = true;
    this.prevVScroll = 0;
  }
  ...
  componentDidMount() {
    ...
    if (width === undefined && this.container.current) {
      this.resizeObserver.observe(this.container.current);
    }

    // Precise supplement to the componentDidUpdate heuristic gate - only relevant when
    // rows can actually carry content-visibility:auto (see .mclContentVisibility in
    // MCLRenderer.css). Harmless to attach otherwise: the event simply never fires when
    // no element has content-visibility:auto applied.
    if (experimentalContentVisibility && !virtualize && this.rowContainer.current) {
      this.rowContainer.current.addEventListener(
        'contentvisibilityautostatechange',
        this.debouncedMeasureHeaderAndScrollbar
      );
    }
    ...
    if (contentData.length > 0) {
      ...
      // based on the width of the containing element. One-time synchronous read for a
      // correctly-sized first paint; the resizeObserver above owns all subsequent changes.
      if (width === undefined && this.container.current && this.container.current.parentNode !== null) {
        newState = this.updateWidthsFromContainerWidth(this.container.current.offsetWidth);
      }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const { columns, isSparse, columnWidths: stateColumnWidths, loading } = this.state;
    ...
    if (shouldUpdate.changed && shouldUpdate.changeType !== CHANGE_NEWDATA) {
      ...
      if (!isNil(this.focusTargetIndex) && !loading) {
        ...
        if (target) {
          const inner = withContentVisibilityVisible(target, () => getNextFocusable(target, true, true));
    ...
    if ((prevProps.width !== this.props.width) || (prevProps.height !== this.props.height)) {
      // keep the resizeObserver in sync with whether we actually need it - width becoming
      // explicit means we no longer need to measure the container ourselves, and vice versa.
      if (prevProps.width !== this.props.width) {
        if (this.props.width === undefined && this.container.current) {
          this.resizeObserver.observe(this.container.current);
          if (this.container.current.parentNode !== null) {
            newState = Object.assign(newState, this.updateWidthsFromContainerWidth(this.container.current.offsetWidth));
          }
        } else if (this.props.width !== undefined && this.container.current) {
      ...
      if (contentData.length !== 0 &&
        !virtualize) {
        // after a resize, we need to make sure the itemToView
        // is scrolled into view again after columns are sized
        // so we set this stale flag to false.
        this.shouldScrollToItemToView = true
      }
    }
    ...
    if (contentData.length > 0) {
      ...
      let averageHeightJustSet = false;
      if (this.rowHeightCache.length > 0 && this.state.averageRowHeight === 0) {
        const avg = this.updateAverageHeight();
        if (avg > 0) {
          newState = Object.assign(newState, { averageRowHeight: avg });
          averageHeightJustSet = true;
        }
      }

      // update header row and check for vertical scrollbar - but only re-measure (a forced
      // layout read of rowElem.offsetWidth/the scroll container's scrollHeight) when
      // something that could plausibly affect row width or scrollbar presence actually
      // changed since the last update, rather than on every render regardless of cause.
      // Unconditional re-measurement here was previously harmless, but now also forces
      // rendering of any content-visibility-skipped rows within the scroll container
      // (see experimentalContentVisibility in MCLRenderer.css).
      // shouldUpdate.changed covers same-length data changes too (e.g. sorting, which
      // triggers a same-cycle column-width recalc in newState above that stateColumnWidths
      // won't reflect until the *next* update, since state hasn't committed yet).
      const columnWidthsChanged = !isEqual(prevState.columnWidths, stateColumnWidths);
      const dimensionsChanged = prevProps.width !== this.props.width || prevProps.height !== this.props.height;

      if (shouldUpdate.changed || columnWidthsChanged || dimensionsChanged || averageHeightJustSet) {
        const scrollbarUpdate = this.measureHeaderAndScrollbar();
        if (scrollbarUpdate) {
          newState = Object.assign(newState, scrollbarUpdate);
        }
      }
    }
  ...
  componentWillUnmount() {
    ...
    this.debouncedMeasureHeaderAndScrollbar.cancel();
    if (this.rowContainer.current) {
      this.rowContainer.current.removeEventListener(
        'contentvisibilityautostatechange',
        this.debouncedMeasureHeaderAndScrollbar
      );
    }
  ...
  calculateInnerItemToView = (targetElem) => {
    ...
    if (mclRect) {
      const rowRect = withContentVisibilityVisible(rowElem, () => rowElem.getBoundingClientRect());
      this.itemToView = {
  ...
  scrollToItemToView = () => {
    ...
    if (item &&
      ...
      if (itemElem) {
        const itemRect = withContentVisibilityVisible(itemElem, () => itemElem.getBoundingClientRect());
        const mclRect = this.scrollContainer.current?.getBoundingClientRect();
        ...
        this.shouldScrollToItemToView = false;
  ...
  // Takes a raw container width - measured either synchronously once at mount, or
  // delivered asynchronously by this.resizeObserver for every change thereafter - and
  // returns a state-shaped partial object (or {} if the width hasn't changed enough to
  // warrant a recompute). Kept separate from the measurement itself so it can be driven
  // by either source without duplicating the calculation.
  updateWidthsFromContainerWidth = (containerWidth) => {
  ...
  // ResizeObserver callback (debounced, see constructor) - the sole post-mount trigger
  // for container-width changes, regardless of cause (prop change, column/data change,
  // or an ambient layout shift elsewhere in the app that a window-resize listener could
  // never catch). Reads the width straight from the observer entry rather than
  // re-measuring offsetWidth, since the observer's own async measurement pass doesn't
  // force a synchronous layout read of a subtree that may contain content-visibility-
  // skipped rows.
  handleContainerResize = (entries) => {
    if (this.props.width !== undefined) return;
    const entry = entries[entries.length - 1];
    const containerWidth = entry.borderBoxSize?.[0]?.inlineSize ?? entry.contentRect.width;
    const newState = this.updateWidthsFromContainerWidth(containerWidth);
    if (Object.keys(newState).length > 0) {
      this.setState(newState);
    }
  }

  // Measures the header row's width (to account for a real scrollbar) and whether the
  // scroll container currently has one. Called both from componentDidUpdate's heuristic
  // gate and from the debounced contentvisibilityautostatechange handler (see constructor/
  // componentDidMount) - returns a state-shaped partial object (or null) rather than
  // calling setState directly, so componentDidUpdate can batch it into its own newState.
  measureHeaderAndScrollbar = () => {
    if (!this.rowContainer.current || !this.headerRow.current) return null;
  ...
  getRowClass = (rowIndex) => {
    ...
    return classnames(
      ...
      // Experimental opt-in - see .mclContentVisibility in MCLRenderer.css. Only takes
      // effect in non-virtualized (static) mode, where MCL doesn't self-measure row
      // height/position; virtualize=true is unaffected regardless of this flag.
      { [css.mclContentVisibility]: this.props.experimentalContentVisibility && !this.props.virtualize }
    );
  };
