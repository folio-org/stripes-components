...
class MCLRenderer extends React.Component {
  ...
  renderCells = (rowIndex) => {
    const { formatter, contentData, columnOverflow, id, columnWidths: columnWidthsProp, borderWithGaps } = this.props;
    ...
    columns.forEach((col) => {
      ...
      const borderStyle = borderWithGaps ? css.borderWithGaps : null;
      ...
      cells.push((
        ...
        >
          <div
            ...
            className={`${css.mclCell} ${showOverflow} ${borderStyle}`}
            style={cellStyle}
          >
            {value}
          </div>
