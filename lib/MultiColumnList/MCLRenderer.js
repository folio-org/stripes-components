...
import { scrollable } from '@bigtest/interactor';
...
class MCLRenderer extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    const { columnWidths, width, contentData } = this.props;
    ...
    if (Object.keys(newState).length > 0) {
      ...
      this.setState(curState => {
        ...
        if (newState.columnWidths && Object.keys(newState.columnWidths).length !== this.state.columns.length) {
  ...
  */
  ...
  getScrollableStyle = () => {
    ...
    */
    if ((bodyHeight && bodyHeight < heightVar) || !heightVar) {
