...
class MCLRenderer extends React.Component {
  ...
  getCanGoNext = () => {
    ...
    let _canGoNext = !contentData[contentData.length - 1];
    if (totalCount) {
      _canGoNext = contentData.length !== totalCount || !contentData[contentData.length - 1];
    } else {
      _canGoNext = !contentData[contentData.length - 1];
    }
