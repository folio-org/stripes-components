...
class MCLRenderer extends React.Component {
  static propTypes = {
    ...
    nonInteractiveHeaderColumns: PropTypes.arrayOf(PropTypes.string),
  ...
  renderCells = (rowIndex) => {
    ...
    const cells = [];
    const labelStrings = [];
    const cellStyleClass = typeof getCellClass === 'function' ? getCellClass(css.mclCellStyle) : css.mclCellStyle;
  ...
  getHeaderClassName = (column, sortedBy, isClickable) => {
    return classnames(
      css.mclHeader,
      { [`${css.mclClickable}`]: isClickable },
  ...
  renderHeaders = () => {
    ...
    columns.forEach((header, i) => {
      ...
      const isClickable = onHeaderClick && !nonInteractiveHeaderColumns.includes(header);
      ...
      headers.push(
        ...
        >
          <div
            role="columnheader"
            className={this.getHeaderClassName(header, isSortHeader, isClickable)}
