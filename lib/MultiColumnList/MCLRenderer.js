...
class MCLRenderer extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (contentData && contentData.length !== 0) {
      /* if pagingType is 'prev-next', contentData can be
      of length pageAmount, so we cannot depend on contentData array
      to calculate dataStartIndex and dataEndIndex */
      if (pagingType === pagingTypes.PREV_NEXT) {
        const maxItemsLength = currentPage * pageAmount;

        newState.dataStartIndex = (currentPage - 1) * pageAmount + 1;
        newState.dataEndIndex = maxItemsLength >= totalCount
          ? totalCount
          : maxItemsLength;
      } else {
        const dataKeys = Object.keys(contentData);

        newState.dataStartIndex = parseInt(dataKeys[0], 10) + 1;
        newState.dataEndIndex = parseInt(dataKeys[dataKeys.length - 1], 10) + 1;
      }
  ...
  renderPagingRow = (rowIndex) => {
    ...
    if (pagingType === pagingTypes.LOAD_MORE) {
      ...
      const canGoNext = currentPage * pageAmount < totalCount;
