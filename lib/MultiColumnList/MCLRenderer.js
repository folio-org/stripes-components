...
class MCLRenderer extends React.Component {
  ...
  handleInfiniteScroll = (currentScroll, currentScrollLeft, index) => {
    ...
    const { scrollTop, maxScrollDelta, scrollDirection, averageRowHeight } = this.state;
    const nextState = {
    ...
    */
    ...
    for (
      ...
      if (contentData[rowIndex] && rowIndex < contentData.length) {
        ...
        const loaderClassname = `${css.mclRow} ${css.mclRowFormatterContainer} ${rowIndex % 2 !== 0 ? '' : css.mclIsOdd}`;
  ...
  renderCells(rowIndex) {
    ...
    this.state.columns.forEach((col) => {
      ...
      let cellStyle = null;
      if (columnWidths) {
        if (isNaN(this.state.columnWidths[col])) {
          console.log('NaN width'); // eslint-disable-line
        } else {
