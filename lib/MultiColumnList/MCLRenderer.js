...
class MCLRenderer extends React.Component {
  ...
  componentDidMount() {
    ...
    if (contentData.length > 0) {
      ...
      // pick up the true header/pagination row heights now that the DOM has laid out,
      // so the very first post-mount render uses correctly reserved space.
      Object.assign(newState, this.measureHeaderAndPaginationHeights());
  ...
  componentDidUpdate(prevProps) {
    ...
    if (contentData.length > 0) {
      // header/pagination row heights are used to calculate the height of the scrollContainer;
      // measureHeaderAndPaginationHeights() only returns keys whose measured value actually
      // changed, so this never forces an extra render on its own.
      Object.assign(newState, this.measureHeaderAndPaginationHeights());
  ...
  measureHeaderAndPaginationHeights = () => {
    const { headerHeight, paginationHeight } = this.state;
    const newState = {};

    if (this.headerRow.current) {
      const measuredHeaderHeight = this.headerRow.current.offsetHeight;
      if (measuredHeaderHeight !== headerHeight) {
        newState.headerHeight = measuredHeaderHeight;
      }
    }

    if (this.paginationContainer.current) {
      const measuredPaginationHeight = this.paginationContainer.current.offsetHeight;
      if (measuredPaginationHeight !== paginationHeight) {
        newState.paginationHeight = measuredPaginationHeight;
      }
    }
  ...
  getRowContainerStyle = () => {
    ...
    const { receivedRows, rowWidth, averageRowHeight, headerHeight, paginationHeight } = this.state;
    ...
    */
    if (height !== undefined) {
      ...
      const scrollBarHeight = 20;
      const effectivePaginationHeight = pagingType === pagingTypes.PREV_NEXT ? paginationHeight : 0;

      newHeight = Math.max(
        ((pagingType !== pagingTypes.SCROLL ? receivedRows : totalCount || receivedRows) * minimumRowHeight),
        (height - headerHeight - scrollBarHeight - effectivePaginationHeight)
      );
  ...
  getScrollableStyle = () => {
    ...
    const { bodyHeight, prevHeight, headerHeight, paginationHeight } = this.state;
    ...
    */
    ...
    if (bodyHeight && maxHeight) {
      scrollableStyle.height = Math.min(bodyHeight, maxHeight - headerHeight - effectivePaginationHeight);
    } else if (heightVar > 0) { // if we've got a positive height prop, use it...
      scrollableStyle.height = heightVar - headerHeight - effectivePaginationHeight;
    }
    ...
    if (maxHeight) {
      scrollableStyle.maxHeight = maxHeight - headerHeight;
    }
