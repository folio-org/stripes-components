...
class MCLRenderer extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (shouldUpdate.changed && shouldUpdate.changeType !== CHANGE_NEWDATA) {
      ...
      if (this.focusTargetIndex !== undefined) {
        ...
        } else if (current && pagingType === pagingTypes.PREV_NEXT) {
          // inconsistent focus on paging can occur if 'data-row-index' is missing or the
          // element cannot be found, so in this case we just focus the first focusable element
          // within the scrollable container...
          const elem = getNextFocusable(current, true, true);
          if (elem) elem.focus();
          this.focusTargetIndex = null;
        }
      }
  ...
  renderDataRow = (rowIndex) => {
    ...
    */
    const defaultStyle = {
      minWidth: `${Math.max((widthVar - (hasMargin ? ROWMARGIN + SCROLLBAR_WIDTH : 0)), rowWidthVar)}px`,
    };
    ...
    if (pagingType === pagingTypes.LOAD_MORE) {
      ...
      return (
        <PrevNextPaginationRow
          ...
          paginationBoundaries={paginationBoundaries !== undefined ? paginationBoundaries : true} // deprecated
        />
      );
    } else {
      return null;
