...
import Cache from './deferredCache';
...
function comparison(obj, oth) {
  ...
  if (objIsArray && objIsArray === Array.isArray(oth)) {
...
class MCLRenderer extends React.Component {
  constructor(props) {
    ...
    this.positionCache = new Cache(24, 0, 0);
    this.rowHeightCache = new Cache(24, 0, 0);
    this.headerWidths = new Cache(null);
    this.widthCache = {};
    ...
    this.throttledHandleScroll = throttle(this.handleInfiniteScroll.bind(this), 64);
      ...
      if (prevProps.width || prevProps.height) {
        if (this.positionCache.length > 1) {
          ...
          if (this.focusedRowIndex !== null) {
            /* if we're not on the last 'page' of results, adjust the firstIndex/scrollTop to keep the focused row
              present
            */
            if (this.state.firstIndex < this.props.totalCount - this.rowCount) {
              newState.firstIndex = this.focusedRowIndex === 0 ? this.focusedRowIndex : this.focusedRowIndex - 1;
              newBase = newState.firstIndex;
              const newScroll = this.positionCache.request(newState.firstIndex);
              if (this.scrollContainer.current) {
                this.scrollContainer.current.scrollTop = newScroll;
              }
              newState.scrollTop = newScroll;
            }
          }
  ...
  handleInfiniteScroll = (currentScroll, currentScrollLeft, index) => {
    const nextState = {
      ...
      scrollDirection: this.state.scrollTop > currentScroll ? 'down' : 'up'
    };

    // look before is a minimumRow + a maximumRowHeight before the currentScroll.
    if (!index) {
      const startPosition = currentScroll - (this.props.minimumRowHeight + this.maximumRowHeight);
  ...
  */
  renderRows() {
    ...
    */
    bodyExtent += (minimumRowHeight + this.maximumRowHeight);
    // let rendered = 0;
    ...
    for (
      ...
      if (contentData[rowIndex]) {
        ...
        rowsRendered += 1;
      } else if (loaderSettings.load && (totalCount ? rowIndex < totalCount : true)) {
        rows.push(
          <RowPositioner
          ...
          return (
            <div data-end-of-list={endIndex} style={endOfListContainerStyle}>
              <EndOfList
                ...
                visible={virtualize && (totalCount && totalCount <= renderPosition)}
              />
            </div>
          );
        }}
      ...
      cells.push((
        ...
        >
          <CellMeasurer
            ...
            rowIndex={rowIndex}
            columnName={col}
            measure={prevWidth && (!columnWidths || !columnWidths[col])}
  ...
  maybeUpdateWidths(rowIndex, columnName, width) {
    ...
    const sample = Math.min(contentData.length, 20);
    if (this.widthCache[columnName].length >= sample && !columnWidths[columnName]) {
  ...
  generateHeaders() {
    ...
    this.state.columns.forEach((header, i) => {
      if (!this.widthCache[header]) {
        this.widthCache[header] = new Cache(null, false);
      }
        ...
        >
          ...
          >
            <CellMeasurer
              ...
              rowIndex={header}
              columnName={header}
              measure={prevWidth && !this.headerWidths.request(header)}
            >
