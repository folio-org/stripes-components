...
class MCLRenderer extends React.Component {
  ...
  */
    ...
    for (
      ...
      if (contentData[rowIndex]) {
          ...
          >
            { /*  rowIndex passed through to children as localRowIndex since
              ...
              ({ localRowIndex, position }) => {
                ...
                if (!virtualize || !prevHeight) measurerStyle = { position: 'static' };
                return (
                  <RowMeasurer
                    heightCache={this.rowHeightCache}
                    style={measurerStyle}
        ...
        if (virtualize && !totalCount) {
          ...
          this.handleLoadMore(30, rowIndex);
        } else {
          rows.push(
            <RowPositioner
  ...
  getColumnWidthsWithUnit = () => {
    const { columnWidths } = this.state;
    const widths = {};
    for (const f in columnWidths) {
      if (Object.prototype.hasOwnProperty.call(columnWidths, f)) {
        widths[f] = `${columnWidths[f]}px`;
      }
    }
    return widths;
  }
  ...
  getRowContainerStyle = () => {
    ...
    const { receivedRows, rowWidth, prevHeight } = this.state;

    let position = 'static';
    if (this.props.virtualize && prevHeight) {
      position = 'relative';
    }
    ...
    */
    if (height !== undefined) {
      ...
      if (totalCount) {
        newHeight = Math.max((totalCount * minimumRowHeight), this.containerHeight || height || maxHeight);
        return { height: `${newHeight}px`, width, position };
      } else {
        newHeight = Math.max((receivedRows * minimumRowHeight), this.containerHeight || height || maxHeight);
        return { height: `${newHeight}px`, width, position };
      }
    }
    return { position };
  ...
  setBodyHeight = (cacheResult) => {
    ...
    const { prevHeight, bodyHeight } = this.state;
    ...
    if (!prevHeight && bodyHeight > this.state.bodyHeight) {
      this.setState({
        bodyHeight: result
      });
    }
  ...
  getScrollableStyle = () => {
    ...
    if (heightVar) {
      ...
      */
      scrollableStyle.height = bodyHeight;
    }
