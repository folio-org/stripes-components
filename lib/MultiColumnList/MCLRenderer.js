...
// possibly used in height calculation:
const PAGING_HEIGHT = 39;
...
class MCLRenderer extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (shouldUpdate.changed && shouldUpdate.changeType !== CHANGE_NEWDATA) {
      ...
      if (this.focusTargetIndex !== undefined) {
        ...
        let target;
        if (this.focusTargetIndex === 'top') {
          target = current && current.querySelector('[role=row]');
        } else {
          target = current && current.querySelector(`[data-row-index="row-${this.focusTargetIndex}"]`);
        }
  ...
  getScrollableStyle = () => {
    ...
    let heightAdjustment = this.headerHeight;
    if (pagingType === pagingTypes.PREV_NEXT || pagingType === pagingTypes.LOAD_MORE) {
      heightAdjustment += PAGING_HEIGHT;
    }
    ...
    */
    ...
    if (bodyHeight && maxHeight) {
      scrollableStyle.height = Math.min(bodyHeight, maxHeight - heightAdjustment);
    } else if (heightVar > 0) { // if we've got a positive height prop, use it...
      scrollableStyle.height = heightVar - heightAdjustment;
    }
      ...
      case pagingTypes.LOAD_MORE:
        PagingComponent = (
          <LoadMorePaginationRow
            ...
            prevWidth={prevWidth}
            rowContainerElement={this.rowContainer.current}
