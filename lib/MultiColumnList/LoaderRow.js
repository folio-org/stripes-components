...
class LoaderRow extends Component {
  ...
  render() {
    const { className, height, columns, visible, columnWidths, styleTop, rowIndex } = this.props;
    if (visible) {
      return (
        <div data-loader-row-index={rowIndex} className={css.mclLoaderRow} style={{ height, position: 'absolute', top: `${styleTop}px` }}>
          <div
            data-loader
            className={className}
            style={{ height }}
